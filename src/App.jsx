...
function projectTasks(wx, hist) {
  ...
  tasks.push({
    ...
    desc: paintReady
      ? ("Good window: " + Math.round(tmp) + "F, " + Math.round(hum) + "% humidity, pressure stable. Check surface temp and avoid direct afternoon sun on dark surfaces.")
      : tmp<50
      ? (Math.round(tmp) + "F is below the 50F minimum for most latex paints. Paint will not cure properly and will peel.")
      : hum>70
      ? ("Humidity at " + Math.round(hum) + "% - paint will not dry evenly above 70%. Wait for drier conditions.")
      : rainComing
      ? "Pressure falling - rain likely within 12-24hrs. Paint needs 24hrs dry time minimum."
      : "Conditions marginal. Need 50-90F, humidity under 70%, no rain 24hrs before or after.",
  ...
  tasks.push({
    ...
    desc: caulkReady
      ? "Good caulking conditions. Clean and dry the surface first. Silicone caulks need 30 min before rain; polyurethane needs 4-8hrs."
      : tmp<40
      ? "Below 40F silicone caulk will not cure properly and loses adhesion. Most products require 40F+ for 24hrs."
  ...
  tasks.push({
    ...
    desc: concreteReady
      ...
      : tmp<50
      ? "Concrete will not cure properly below 50F. Cold temps cause cracking and weak final strength."
      : rainComing
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        {[["lawn","Lawn" + (readyLawn>0?" ("+readyLawn+")":"")],["projects","Projects" + (readyProj>0?" ("+readyProj+")":"")],["compost","Compost"]].map(([id,lbl])=>(
