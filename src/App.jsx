...
function PlantJournalSheet({ plant, allPlants, weather, onClose, onNewPlantDetected, onOpenChat }) {
  ...
  function saveEntry(entry) {
    if (!plant.id) { console.error("Plant has no id — cannot save journal entry"); return; }
    setEntries(prev => {
      const u = [entry, ...prev];
      saveCritical(key, u);  // writes to both localStorage AND IndexedDB
      return u;
    });
    // Also mirror to global journal
    const global = loadLS("pl_journal",[]);
    saveCritical("pl_journal", [{ ...entry, plantId: plant.id, plantName: plant.name }, ...global]);
    setShowForm(false);
    setSavedMsg(true);
        ...
        <div style={{padding:"20px 20px 12px",borderBottom:`1px solid ${C.sep}`,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{fontSize:10,color:C.textDim,fontFamily:F.body,marginTop:1}}>id: {plant.id||"⚠ MISSING"}</div>
