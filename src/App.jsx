      ...
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",background:C.surface,borderRadius:"20px 20px 0 0",height:"88vh",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"16px 20px 40px"}}>
          ...
          <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:20,marginBottom:20}}>
            ...
            {/* ── Plant Storage Diagnostic ── */}
            {(()=>{
              const allPlants = loadLS("pl_plants",[]);
              if (!allPlants.length) return null;
              // Check for duplicate names
              const nameCounts = {};
              allPlants.forEach(p=>{ nameCounts[p.name] = (nameCounts[p.name]||0)+1; });
              const hasDupes = Object.values(nameCounts).some(n=>n>1);
              return (
                <div style={{marginTop:14,borderTop:`1px solid ${C.sep}`,paddingTop:14}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <div style={{fontSize:13,fontWeight:"600",color:C.text,fontFamily:F.body}}>Plant Storage Check</div>
                    {hasDupes && <div style={{fontSize:11,color:C.danger,fontFamily:F.body,fontWeight:"600"}}>⚠ DUPLICATES FOUND</div>}
                  </div>
                  {allPlants.map(p=>{
                    const entries = loadLS("pl_pjournal_"+p.id,[]);
                    const isDupe = nameCounts[p.name] > 1;

                    // Scan ALL localStorage keys for any journal that might belong to this plant
                    const orphanedKeys = [];
                    try {
                      for (let i=0; i<localStorage.length; i++) {
                        const k = localStorage.key(i);
                        if (k && k.startsWith("pl_pjournal_") && k !== "pl_pjournal_"+p.id) {
                          const data = loadLS(k,[]);
                          if (data.length > 0) {
                            orphanedKeys.push({key:k, count:data.length});
                          }
                        }
                      }
                    } catch {}
                    ...
                    return (
                      <div key={p.id} style={{padding:"8px 0",borderBottom:`1px solid ${C.sep}`,background:isDupe?"#FFF2F2":"transparent"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div>
                            <div style={{fontSize:13,color:isDupe?C.danger:C.text,fontFamily:F.body,fontWeight:isDupe?"600":"400"}}>{p.emoji} {p.name}{isDupe?" ⚠ DUPLICATE":""}</div>
                            <div style={{fontSize:10,color:C.textDim,fontFamily:F.body}}>id: {p.id} · key: pl_pjournal_{p.id}</div>
                          </div>
                        ...
                        {orphanedKeys.length>0 && (
                          <div style={{marginTop:6,fontSize:11,color:C.warn,fontFamily:F.body,background:"#FFF9F0",padding:"6px 8px",borderRadius:6}}>
                            ⚠ Found other journal keys in storage: {orphanedKeys.map(o=>`${o.key} (${o.count} entries)`).join(", ")}
                          </div>
                        )}
                      </div>
                    );
                  ...
                  {hasDupes && (
                    <div style={{marginTop:10,fontSize:12,color:C.danger,fontFamily:F.body,lineHeight:1.6,padding:"8px 10px",background:"#FFF2F2",borderRadius:8}}>
                      Duplicate plants found. You may be saving entries to one copy and reading from another. Delete the duplicate with 0 entries from the Garden tab.
                    </div>
                  )}
                </div>
              );
