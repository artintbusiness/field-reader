...
function saveLS(k, v) {
  try {
    ...
    console.error("localStorage write failed for key:", k, e);
  }
}

// ─── INDEXED DB BACKUP ────────────────────────────────────────────────────────
// iOS Safari can clear localStorage for PWAs. We mirror critical plant journal
// data to IndexedDB which is more persistent, and restore from it on load.
const IDB_NAME = "PlotDB", IDB_STORE = "journal", IDB_VER = 1;
...
// Save to BOTH localStorage and IndexedDB for critical keys
async function saveCritical(k, v) {
  saveLS(k, v);
  await idbSet(k, JSON.stringify(v));
}

// Load from localStorage, fall back to IndexedDB if missing
async function loadCritical(k, def) {
  const ls = loadLS(k, null);
  if (ls !== null) return ls;
  // localStorage was cleared — try IndexedDB
  const idb = await idbGet(k);
  if (idb !== null) {
    try {
      const parsed = JSON.parse(idb);
      saveLS(k, parsed); // restore to localStorage
      console.log("Restored from IndexedDB:", k);
      return parsed;
    } catch {}
  }
  return def;
...
function pressureTrend(history) {
  ...
  // Use last 6 hours of readings — time-based so AWN history density doesn't distort it
  const window = history.filter(s => s.ts >= now - 6*60*60*1000);
  const r = window.length >= 3 ? window : history.slice(-12);
...
function windShift(history) {
  ...
  const now = history[history.length-1]?.ts || Date.now();
  // Use last 2 hours — time-based so AWN history density doesn't distort it
  const window = history.filter(s => s.ts >= now - 2*60*60*1000 && s.windDir);
  const r = window.length >= 3 ? window : history.slice(-8).filter(s=>s.windDir);
  if (r.length < 2) return null;
  const oldest = r[0]?.windDir;
  const newest = r[r.length-1]?.windDir;
  ...
  // Veering = clockwise rotation (positive diff)
  // Backing  = counter-clockwise rotation (negative diff)
  const type = diff > 0 ? "veering" : "backing";
  const deg  = Math.abs(Math.round(diff));
  // Describe what the direction change actually means without oversimplified good/bad
  const towardsSouth = d2 > 90 && d2 < 270;
  const towardsNorth = d2 <= 90 || d2 >= 270;
  const context = towardsSouth
    ? "wind shifting toward warmer/moister sector"
    : "wind shifting toward cooler/drier sector";
  return {
    type,
    from:  oldest,
    to:    newest,
    deg,
    label: `Wind ${type} ${oldest}→${newest} (${deg}°) — ${context}`,
    color: C.warn,  // always amber — any significant shift is worth noting
  };
}
function frostRisk(tempF, hum, wind) {
  ...
  if (tempF <= 36)             return { level:"caution", label:"Frost Possible", color: C.warn,     desc:"Near frost threshold — monitor overnight lows." };
  if (tempF <= 40)             return { level:"safe",    label:"Frost Watch",    color: C.textMid,  desc:"Below 40F but above frost threshold." };
...
const PLANT_QUICK_TAGS = [
  {l:"New growth",e:"🌱"},{l:"Flowering",e:"🌸"},{l:"Fruiting",e:"🍅"},
  {l:"Yellowing leaves",e:"🟡"},{l:"Wilting",e:"😓"},{l:"Pest damage",e:"🐛"},
  {l:"Fungal spots",e:"🍄"},{l:"Pruned",e:"✂️"},{l:"Watered",e:"💧"},
  {l:"Fertilized",e:"🌿"},{l:"Repotted",e:"🪴"},{l:"Transplanted",e:"🌍"},
];

function JournalEntryForm({ weather, plantContext, allPlants, onSave, onCancel, onNewPlantDetected }) {
  const isPlant = !!plantContext;
  const [fields, setFields]           = useState({ notes:"", appearance:"", action:"" });
  const [qTags,  setQTags]            = useState([]);
  const [photo, setPhoto]             = useState(null);
  const [photoPreview, setPhotoPreview] = useState(null);
  const [saving, setSaving]           = useState(false);
  const [detectedPlants, setDetectedPlants] = useState([]); // track for tagging
  ...
  const dictation = useDictation({
    ...
    onParsed: (parsed) => {
      setFields(f => ({
        ...
        appearance: parsed.clouds || f.appearance,
        action:     parsed.signs  || f.action,
      }));
      if (parsed.detectedPlants?.length) {
        setDetectedPlants(parsed.detectedPlants);
      }
  ...
  function buildEntry() {
    ...
    return {
      ...
      note:    capFirst(fields.notes),
      clouds:  fields.appearance,
      signs:   [fields.action, allTags].filter(Boolean).join(", "),
      photo:   photo,
      _detectedPlants: detectedPlants, // passed to saveGeneralEntry for routing + tagging
    };
  }

  function canSave() {
    return fields.notes.trim() || fields.appearance.trim() || fields.action.trim() || qTags.length || photo;
  }
  ...
  return (
    <div style={{...css.card, borderColor:`${C.accent}44`, marginBottom:16}}>
      ...
      {/* ── Quick tags — plant or weather context ── */}
      <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:14,marginBottom:14}}>
        <span style={css.label}>{isPlant ? "Quick Tags" : "Quick Signs"}</span>
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {(isPlant ? PLANT_QUICK_TAGS : QUICK_SIGNS).map(s=>(
            <button key={s.l} onClick={()=>setQTags(p=>p.includes(s.l)?p.filter(x=>x!==s.l):[...p,s.l])} style={{padding:"5px 10px",borderRadius:18,fontSize:12,border:`1px solid ${qTags.includes(s.l)?C.accent:C.sep}`,cursor:"pointer",fontFamily:F.body,background:qTags.includes(s.l)?C.accentDim:"transparent",color:qTags.includes(s.l)?C.accent:C.textMid}}>
              {s.e} {s.l}
            </button>
          ))}
        </div>
      ...
      <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:14}}>
        {isPlant ? (
          <>
            <div style={{fontSize:12,color:C.textMid,marginBottom:4,fontFamily:F.body}}>Appearance</div>
            <input value={fields.appearance} onChange={e=>setFields(f=>({...f,appearance:e.target.value}))} placeholder="e.g. Leaves dark green, new shoot on main stem..." style={css.input}/>
            <div style={{fontSize:12,color:C.textMid,marginBottom:4,fontFamily:F.body}}>Action taken</div>
            <input value={fields.action} onChange={e=>setFields(f=>({...f,action:e.target.value}))} placeholder="e.g. Watered, pruned lower leaves, added stake..." style={css.input}/>
          </>
        ) : (
          <>
            <div style={{fontSize:12,color:C.textMid,marginBottom:4,fontFamily:F.body}}>Clouds / Sky</div>
            <input value={fields.appearance} onChange={e=>setFields(f=>({...f,appearance:e.target.value}))} placeholder="e.g. Cumulus building to the west..." style={css.input}/>
            <div style={{fontSize:12,color:C.textMid,marginBottom:4,fontFamily:F.body}}>Nature Signs</div>
            <input value={fields.action} onChange={e=>setFields(f=>({...f,action:e.target.value}))} placeholder="e.g. Robins active, earthy smell after rain..." style={css.input}/>
          </>
        )}
        <div style={{fontSize:12,color:C.textMid,marginBottom:4,fontFamily:F.body}}>
          {isPlant ? "Observations" : "Notes"}
        </div>
        <textarea
          value={fields.notes}
          onChange={e=>setFields(f=>({...f,notes:e.target.value}))}
          placeholder={isPlant
            ? `What did you notice about ${plantContext.plant.name} today?`
            : "What did you observe, do, or notice?"}
          rows={4}
          style={{...css.input, resize:"vertical"}}
        />
      </div>

      <div style={{display:"flex",gap:10,marginTop:4,paddingBottom:"calc(env(safe-area-inset-bottom,0px) + 80px)"}}>
...
function EntryCard({ entry: e, onDelete, onUpdate }) {
  ...
  const [photoSrc, setPhotoSrc] = useState(e.photo && !e.photo.startsWith("idb:") ? e.photo : null);

  useEffect(()=>{
    if (e.photo && e.photo.startsWith("idb:")) {
      const photoId = e.photo.replace("idb:","");
      idbGet("photo_"+photoId).then(data=>{ if(data) setPhotoSrc(data); });
    }
  },[e.photo]);
  ...
  if (editing) {
    return (
      <div style={{...css.card, borderColor:`${C.accent}44`}}>
        ...
        {photoSrc && <img src={photoSrc} style={{width:"100%",borderRadius:10,marginBottom:10,maxHeight:160,objectFit:"cover"}}/>}
        ...
        <div style={{display:"flex",gap:10,paddingBottom:"calc(env(safe-area-inset-bottom,0px) + 80px)"}}>
  ...
  return (
    <div style={css.card}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
            ...
            {/* Single plant tag */}
            {e.plantName && !e.plantNames?.length && (
              <div style={{fontSize:11,color:C.surface,background:C.accent,borderRadius:980,padding:"1px 8px",fontFamily:F.body,fontWeight:"600"}}>
                {e.plantName}
              </div>
            )}
            {/* Multiple plant tags */}
            {e.plantNames?.map(name=>(
              <div key={name} style={{fontSize:11,color:C.surface,background:C.accentMid,borderRadius:980,padding:"1px 8px",fontFamily:F.body,fontWeight:"600"}}>
                {name}
              </div>
            ))}
          </div>
      ...
      {photoSrc  && <img src={photoSrc} style={{width:"100%",borderRadius:10,marginBottom:8,maxHeight:200,objectFit:"cover"}}/>}
...
// ─── PLANT JOURNAL SHEET ──────────────────────────────────────────────────────
function PlantJournalSheet({ plant, allPlants, weather, onClose, onNewPlantDetected, onOpenChat }) {
  ...
  function saveEntry(entry) {
    if (!plant.id) { alert("ERROR: Plant has no id — cannot save"); return; }
    // Strip photo from localStorage entry — store it in IDB separately
    // This prevents localStorage quota errors from base64 image data
    const photoData = entry.photo || null;
    const entryNoPhoto = { ...entry, photo: photoData ? "idb:"+entry.id : null };
    if (photoData) idbSet("photo_"+entry.id, photoData); // async, non-blocking

    const existing = loadLS(key,[]);
    const u = [entryNoPhoto, ...existing];
    try {
      localStorage.setItem(key, JSON.stringify(u));
      const verify = localStorage.getItem(key);
      if (!verify || !JSON.parse(verify).length) {
        alert("ERROR: Write appeared to succeed but readback shows empty for key: " + key);
        return;
      }
    } catch(e) {
      alert("Error saving entry: " + e.message + " — Go to Settings → Data Backup → Free Up Storage Space, then try again.");
      return;
    }
    // Attach photo back for in-memory display (so it shows immediately)
    const entryWithPhoto = { ...entryNoPhoto, photo: photoData };
    setEntries([entryWithPhoto, ...existing]);
    // Mirror to global journal (also no photo in localStorage)
    const global = loadLS("pl_journal",[]);
    saveLS("pl_journal", [{ ...entryNoPhoto, plantId: plant.id, plantName: plant.name }, ...global]);
    setShowForm(false);
    setSavedMsg(true);
  ...
  function updateEntry(updated) {
    setEntries(prev => {
      const u = prev.map(e=>e.id===updated.id?updated:e);
      saveCritical(key, u);
      return u;
    });
  }
        ...
        <div style={{padding:"20px 20px 12px",borderBottom:`1px solid ${C.sep}`,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{fontSize:10,color:C.textDim,fontFamily:F.body,marginTop:1}}>id: {plant.id||"⚠ MISSING"} · {entries.length} entries in memory</div>
              <div style={{fontSize:10,color:C.textDim,fontFamily:F.body}}>ls: {(()=>{ try { const v=localStorage.getItem(key); return v ? JSON.parse(v).length+" in storage" : "EMPTY"; } catch(e){ return "READ ERROR"; }})()}</div>
              {savedMsg && <div style={{fontSize:12,color:C.accent,fontFamily:F.body,marginTop:2}}>✓ Entry saved</div>}
            </div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <button onClick={()=>setShowForm(o=>!o)} style={{...css.btn,padding:"7px 14px",fontSize:13}}>+ Note</button>
              {onOpenChat && (
                <button onClick={()=>onOpenChat(plant)} style={{...css.btnSoft,padding:"7px 12px",fontSize:13}} title="Ask garden assistant">💬</button>
              )}
...
// ══════════════════════════════════════════════════════════════════════════════
// ─── GARDEN CHAT ──────────────────────────────────────────────────────────────
// Conversational AI plant diagnostic agent.
// Builds context from current conditions + plant journals automatically.
// ══════════════════════════════════════════════════════════════════════════════
function GardenChat({ weather, history, plants, tempOffset=0, forecast=[], focusPlant=null, onClose }) {
  ...
  const humOffset= parseFloat(localStorage.getItem("pl_hum_off")||"0")||0;
  ...
  const water    = adjTemp != null ? wateringAdv({...weather, temperature_2m: adjTemp}, history, humOffset) : null;
  const fung     = adjTemp != null ? fungalRisk({...weather, temperature_2m: adjTemp}, history, humOffset) : null;
  const rain24   = weather?.source==="pws" ? (weather.rain24h||0) : history.slice(-48).reduce((s,r)=>s+(r.precipitation||0),0);
  const dc       = getDateContext();

  // Build rich system prompt from garden context
  function buildSystemPrompt() {
    const conditions = weather ? [
      `Temperature: ${Math.round(adjTemp)}F (yard-adjusted from ${Math.round(weather.temperature_2m)}F raw)`,
      `Humidity: ${Math.round(Math.max(0,weather.relative_humidity_2m-humOffset))}% (adjusted)`,
      `Soil temperature estimate: ${soil}F`,
      `Rain 24hr: ${rain24.toFixed(2)}"`,
      `Wind: ${Math.round(weather.wind_speed_10m)} mph ${windLabel(weather.wind_direction_10m)}`,
      `Frost risk: ${frost?.label} — ${frost?.desc}`,
      `Weather source: ${weather.source==="pws"?"Personal backyard weather station":"Grid model"}`,
    ].join("\n") : "No weather data available.";

    const advisors = water && fung ? [
      `WATERING: ${water.label} — ${water.desc}`,
      `FUNGAL RISK: ${fung.label} — ${fung.desc}`,
      `SOIL TEMP: ${soil}F — ${soil<50?"Too cold for most seeds":soil<60?"Good for brassicas/lettuce":soil<70?"Tomatoes/peppers marginal":soil<80?"Ideal for warm-season crops":"Hot — cool-season stress risk"}`,
    ].join("\n") : "";

    const forecastSection = forecast.length ? forecast.slice(0,10).map((d,i) => {
      const adjMin = d.minF + tempOffset;
      const adjMax = d.maxF + tempOffset;
      const fr = frostRisk(adjMin, 70, 5);
      const label = i===0?"Today":i===1?"Tomorrow":new Date(d.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"});
      const frostFlag = fr.level!=="safe" ? ` ⚠ ${fr.label}` : "";
      return `${label}: high ${Math.round(adjMax)}F / low ${Math.round(adjMin)}F, rain ${d.rain.toFixed(2)}"${frostFlag}`;
    }).join("\n") : "No forecast data available.";

    const plantContext = plants.length ? plants.map(p => {
      const entries = loadLS("pl_pjournal_"+p.id,[]).slice(0,8);
      const notes = entries.length
        ? entries.map(e=>{
            const parts = [e.date];
            if (e.clouds || e.appearance) parts.push("Appearance: "+(e.clouds||e.appearance));
            if (e.signs)   parts.push("Tags/actions: "+e.signs);
            if (e.notes||e.note) parts.push(e.notes||e.note);
            if (e.photo)   parts.push("[photo attached]");
            return "  "+parts.join(" | ");
          }).join("\n")
        : "  No journal entries yet.";
      return `${p.emoji} ${p.name} (${p.location}${p.planted?", planted "+p.planted:""}):\n${notes}`;
    }).join("\n\n") : "No plants registered yet.";

    const focusSection = focusPlant
      ? `\nFOCUS PLANT: The user has opened this chat from ${focusPlant.emoji} ${focusPlant.name}'s journal. Prioritise advice about this plant.`
      : "";

    return `You are a knowledgeable garden and plant diagnostician embedded in the Plot app. You have full access to the user's backyard conditions, garden advisor readings, 10-day forecast, plant journal history, and recent plant photos. Be specific, practical, and terse. Reference plant names, journal observations, advisor readings, and upcoming weather directly. Ask clarifying questions if needed. No em dashes.${focusSection}

TODAY: ${dc.date} — ${dc.season}

CURRENT CONDITIONS:
${conditions}

GARDEN ADVISORS (computed from current conditions):
${advisors}

10-DAY FORECAST (overnight lows yard-adjusted by ${tempOffset>=0?"+":""}${tempOffset.toFixed(1)}F):
${forecastSection}
...
${plantContext}`;
  ...
  async function send(text) {
    ...
    const key = localStorage.getItem("pl_anthropic_key")||"";
    if (!key) { setError("No Anthropic API key — add it in Settings."); return; }
    ...
    // Collect recent photos from plant journals (up to 3, most recent first)
    // Only attach on the first message to avoid bloating the context
    const isFirstMessage = messages.length === 0;
    const recentPhotos = [];
    if (isFirstMessage) {
      const plantsToCheck = focusPlant ? [focusPlant] : plants;
      for (const p of plantsToCheck) {
        const entries = loadLS("pl_pjournal_"+p.id,[]);
        for (const e of entries) {
          if (e.photo && recentPhotos.length < 3) {
            // photo is stored as data:image/jpeg;base64,... — extract base64 part
            const b64 = e.photo.split(",")[1];
            if (b64) recentPhotos.push({ b64, date: e.date, plant: p.name });
          }
          if (recentPhotos.length >= 3) break;
        }
        if (recentPhotos.length >= 3) break;
      }
    }
    ...
    try {
      // Build message content — text + optional photos on first message
      const userContent = recentPhotos.length > 0
        ? [
            ...recentPhotos.map(p => ({
              type: "image",
              source: { type:"base64", media_type:"image/jpeg", data: p.b64 },
            })),
            { type:"text", text: `[Recent photos from journal: ${recentPhotos.map(p=>`${p.plant} (${p.date})`).join(", ")}]\n\n${trimmed}` },
          ]
        : trimmed;
  ...
  const suggestions = [
    ...
    "What's causing the spots on my leaves?",
    "When should I fertilize?",
  ];

  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:600,display:"flex",alignItems:"flex-end"}}>
      <div style={{
        width:"100%",maxWidth:480,margin:"0 auto",
        background:C.surface,borderRadius:"20px 20px 0 0",
        height:"90dvh",          // dvh accounts for iOS browser chrome
        maxHeight:"90dvh",
        display:"flex",flexDirection:"column",
        overflow:"hidden",       // prevent children breaking out
      }}>

        {/* ── Header ── */}
        <div style={{padding:"18px 20px 12px",borderBottom:`1px solid ${C.sep}`,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:17,fontWeight:"700",color:C.text,fontFamily:F.body}}>🌿 Garden Assistant</div>
              <div style={{fontSize:12,color:C.textMid,fontFamily:F.body,marginTop:2}}>
                Ask anything about your plants or garden
              </div>
            </div>
            <button onClick={onClose} style={{background:"none",border:"none",color:C.textMid,fontSize:24,cursor:"pointer",lineHeight:1,padding:"4px 8px"}}>×</button>
          </div>
        </div>

        {/* ── Messages ── */}
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"16px 16px 8px",minHeight:0}}>

          {/* Empty state — suggestions */}
          {messages.length===0 && (
            <div>
              <div style={{textAlign:"center",padding:"16px 0 14px"}}>
                <div style={{fontSize:32,marginBottom:6}}>{focusPlant ? focusPlant.emoji : "🌱"}</div>
                <div style={{fontSize:14,color:C.textMid,fontFamily:F.body,lineHeight:1.6}}>
                  {focusPlant
                    ? `I can see ${focusPlant.name}'s journal notes and recent photos. Ask me anything about this plant.`
                    : "I know your plants, conditions, journal notes, and recent photos. Ask me anything."
                  }
                </div>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {suggestions.map(s=>(
                  <button key={s} onClick={()=>send(s)} style={{
                    ...
                    fontSize:14, color:C.accent, fontFamily:F.body, cursor:"pointer",
                  }}>
                    {s}
                  </button>
                ))}
          ...
          {loading && (
            <div style={{display:"flex",justifyContent:"flex-start",marginBottom:12}}>
              <div style={{background:C.bg,borderRadius:"18px 18px 18px 4px",padding:"10px 16px",display:"flex",gap:4,alignItems:"center"}}>
                {[0,1,2].map(i=>(
                  <div key={i} style={{
                    width:6,height:6,borderRadius:"50%",background:C.accentMid,
          ...
          {/* Error */}
          {error && (
            <div style={{fontSize:12,color:C.danger,background:"#FFF2F2",padding:"8px 12px",borderRadius:10,marginBottom:8,fontFamily:F.body}}>
              ⚠ {error}
            </div>
          )}
        ...
        <div style={{
          ...
          {messages.length>0 && (
            <button
              onClick={()=>setMessages([])}
              style={{background:"none",border:"none",color:C.textDim,fontSize:12,fontFamily:F.body,cursor:"pointer",marginTop:6,padding:0}}
            >
              Clear conversation
            </button>
          )}
...
function Garden({ weather, history, tempOffset=0, forecast=[], onOpenChat }) {
  ...
  // Match plant names from free text — handles partial matches like "pepper" → all pepper variants
  function detectPlantsInText(text) {
    if (!text || !plants.length) return [];
    const lower = text.toLowerCase();
    return plants.filter(p => {
      // Match full name or any word in the plant name
  ...
  function saveGeneralEntry(entry) {
    // Strip photo from localStorage — store in IDB to avoid quota errors
    const photoData = entry.photo || null;
    const entryNoPhoto = { ...entry, photo: photoData ? "idb:"+entry.id : null };
    if (photoData) idbSet("photo_"+entry.id, photoData);

    // Detect plants from BOTH dictation results AND free text scan
    const detectedNames = entry._detectedPlants || [];
    const fullText = [entry.notes, entry.clouds, entry.signs, entry.appearance].filter(Boolean).join(" ");
    const textMatched = detectPlantsInText(fullText);
    // Merge both sources, deduplicate by id
    const allMatched = [...textMatched];
    detectedNames.forEach(name => {
      const p = plants.find(p => p.name.toLowerCase()===name.toLowerCase() ||
        name.toLowerCase().includes(p.name.toLowerCase().split(" ")[0]));
      if (p && !allMatched.find(m=>m.id===p.id)) allMatched.push(p);
    });

    // Tag entry with matched plant names for display
    const taggedEntry = {
      ...entryNoPhoto,
      plantNames: allMatched.length > 1 ? allMatched.map(p=>p.name) : undefined,
      plantName:  allMatched.length === 1 ? allMatched[0].name : undefined,
    };

    const global = loadLS("pl_journal",[]);
    const updated = [taggedEntry, ...global];
    saveLS("pl_journal", updated);
    // Keep photo in memory for immediate display
    setGenJournal(updated.map(e => e.id===taggedEntry.id ? {...taggedEntry, photo: photoData} : e));
    setShowGenJournal(false);

    // Route to each matched plant's journal
    if (allMatched.length) {
      allMatched.forEach(plant => {
        const jkey = "pl_pjournal_"+plant.id;
        const existing = loadLS(jkey,[]);
        saveLS(jkey, [{ ...taggedEntry, plantId: plant.id, plantName: plant.name }, ...existing]);
      });
    }
  }

  // Retroactively tag existing untagged general journal entries
  function retagJournal() {
    const global = loadLS("pl_journal",[]);
    let changed = 0;
    const updated = global.map(e => {
      if (e.plantName || e.plantNames?.length) return e; // already tagged
      const fullText = [e.notes, e.note, e.clouds, e.signs].filter(Boolean).join(" ");
      const matched = detectPlantsInText(fullText);
      if (!matched.length) return e;
      changed++;
      return {
        ...e,
        plantNames: matched.length > 1 ? matched.map(p=>p.name) : undefined,
        plantName:  matched.length === 1 ? matched[0].name : undefined,
      };
    });
    if (changed > 0) {
      saveLS("pl_journal", updated);
  ...
  async function getAdvice() {
    ...
    // Build frost outlook summary from 10-day forecast
    const frostDays = forecast
      .map((d,i) => {
        const adjMin = d.minF + tempOffset;
        const fr = frostRisk(adjMin, 70, 5);
        return fr.level!=="safe" ? `${new Date(d.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}: low ${Math.round(adjMin)}F (${fr.label})` : null;
      })
      .filter(Boolean);
    const frostOutlook = frostDays.length
      ? `FROST RISK IN NEXT 10 DAYS:\n${frostDays.join("\n")}`
      : "No frost risk in the next 10 days.";

    const prompt = `${dc.date} ${dc.time} — ${dc.season}
WEATHER: ${src}
CONDITIONS: ${Math.round(weather.temperature_2m + tempOffset)}F (yard-adjusted), ${Math.round(adjHum)}% humidity, soil ~${soil}F, wind ${Math.round(weather.wind_speed_10m)}mph ${windLabel(weather.wind_direction_10m)}, ${rain24.toFixed(2)}" rain 24hr
FROST TODAY: ${frost?.label} | FUNGAL RISK: ${fung?.label} | WATERING: ${water?.label}
${frostOutlook}

PLANTS AND RECENT JOURNAL NOTES:
${buildJournalContext() || "No plants registered yet."}

You are an expert gardener. Give 3-4 sentences of specific, actionable advice based on the conditions, journal notes, and frost outlook above. If frost is coming in the next few days, factor that into planting, hardening off, or outdoor move timing. Reference specific plant names and recent observations where relevant. End with one concrete action to take today. No em dashes.`;
    try {
...
1) Product name and type  2) Key application requirements from the label  3) Whether conditions meet requirements  4) Rain timing requirements  5) Clear yes/no for today with one-sentence reason.`,
    ...
    try {
      ...
      setPhotoResult(txt);
      // Save to general journal — strip photo to IDB to avoid quota errors
      const entryId = Date.now();
      if (photoB64) idbSet("photo_"+entryId, `data:image/jpeg;base64,${photoB64}`);
      const entry = {
        id: entryId,
        date: new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),
        time: new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"}),
        ...
        photo: photoB64 ? "idb:"+entryId : null,
        clouds:"", signs:"",
  ...
  return (
    <div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom: showGenJournal ? 12 : 8}}>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              ...
              <InfoButton text="For observations not tied to one specific plant. Plant names in your notes are automatically detected and tagged. Tap 'Re-tag' to update tags on existing entries."/>
            </div>
          ...
          <div style={{display:"flex",gap:6}}>
            {genJournal.length>0 && (
              <button onClick={()=>{
                const n = retagJournal();
                if (n===0) alert("All entries are already tagged.");
              }} style={{...css.btnSoft,padding:"7px 12px",fontSize:12}}>Re-tag</button>
            )}
          ...
          <div style={{marginTop: showGenJournal ? 8 : 0}}>
            {(showAllGenJournal ? genJournal : genJournal.slice(0,3)).map(e=>(
              <EntryCard
                ...
                entry={e}
                onDelete={()=>{
                  setGenJournal(prev=>{ const u=prev.filter(x=>x.id!==e.id); saveLS("pl_journal",u); return u; });
                }}
                onUpdate={updated=>{
                  setGenJournal(prev=>{ const u=prev.map(x=>x.id===updated.id?updated:x); saveLS("pl_journal",u); return u; });
                }}
              />
            ))}
            {genJournal.length > 3 && (
      ...
      {/* ── Floating Garden Chat button ── */}
      <div style={{position:"fixed",bottom:"calc(env(safe-area-inset-bottom,0px) + 72px)",right:20,zIndex:400}}>
        <button
          onClick={onOpenChat}
          style={{
...
function evalActivity(id, wx, hist, forecast=[]) {
  ...
  // Forecast rain helpers — check actual forecast data, not just pressure proxy
  const fcRain24 = forecast.slice(0,1).reduce((s,d)=>s+(d.rain||0),0);  // today
  const fcRain48 = forecast.slice(0,2).reduce((s,d)=>s+(d.rain||0),0);  // today + tomorrow
  const fcRain72 = forecast.slice(0,3).reduce((s,d)=>s+(d.rain||0),0);  // 3 days
  const rainToday    = fcRain24 > 0.1;
  const rain2days    = fcRain48 > 0.1;
  const rain3days    = fcRain72 > 0.1;
  const fcRainLabel  = fcRain48 > 0.1
    ? `${fcRain48.toFixed(2)}" forecast in 48hrs`
    : fcRain72 > 0.1
    ? `${fcRain72.toFixed(2)}" forecast in 72hrs`
    : "Clear forecast";
  const conditions = [];
  let verdict = "yes";
  ...
  switch(id) {
    ...
    case "paint":
      ...
      pass("Rain Today",rainToday?fcRainLabel:"Clear",!rainToday,rainToday?"Rain forecast today — needs 24hr dry after":"Good");
      pass("Rain 48hrs",rain2days?fcRainLabel:"Clear",!rain2days,rain2days?"Rain in next 48hrs — paint won't cure fully":"Clear window"); break;
    case "stain":
      pass("Temp",Math.round(tmp)+"F",tmp>=50&&tmp<=90,"Good");
      pass("Humidity",Math.round(hum)+"%",hum<=65,hum>65?"Blotchy finish above 65%":"Good");
      pass("Dry Surface",dryStretch?"Yes":"No",dryStretch,"Need 48hr dry surface");
      pass("Rain 48hrs",rain2days?fcRainLabel:"Clear",!rain2days,rain2days?"Rain forecast — stain needs 48hr dry window":"Good"); break;
    case "caulk":
      pass("Temp",Math.round(tmp)+"F",tmp>=40&&tmp<=90,tmp<40?"Won't cure below 40F":"Good");
      pass("Rain Today",rainToday?fcRainLabel:"Clear",!rainToday,rainToday?"Surface must be completely dry — rain forecast":"Good"); break;
    case "concrete":
      pass("Temp",Math.round(tmp)+"F",tmp>=50&&tmp<=90,tmp<50?"Won't cure — cracking risk":tmp>90?"Surface dries too fast":"Good");
      pass("Rain 48hrs",rain2days?fcRainLabel:"Clear",!rain2days,rain2days?"Do not pour — rain forecast in next 48hrs":"Good window"); break;
    case "powerwash":
      pass("Temp",Math.round(tmp)+"F",tmp>=45,"Good");
      pass("Rain Today",rainToday?fcRainLabel:"Clear",!rainToday,rainToday?"Pointless if rain forecast today":"Good"); break;
    case "shed":
      pass("Temp",Math.round(tmp)+"F",tmp>=45,"Good");
      pass("Rain",rain24.toFixed(2)+'"',rain24<0.5,"Ground too wet for footings");
      pass("Rain Today",rainToday?fcRainLabel:"Clear",!rainToday,rainToday?"Rain forecast — hold off on concrete/footings":"Good"); break;
    case "driveway_seal":
      pass("Temp",Math.round(tmp)+"F",tmp>=50&&tmp<=90,"Good");
      pass("Dry Stretch",dryStretch?"Yes":"No",dryStretch,"Need 2-3 dry days before");
      pass("Rain 72hrs",rain3days?fcRainLabel:"Clear",!rain3days,rain3days?"Rain forecast — sealer needs 48-72hr cure":"Good window"); break;
    case "herbicide":
      pass("Wind",Math.round(wind)+" mph",wind<10,wind>=10?"Drift risk — wait for <10mph":"Good");
      pass("Rain",rain24.toFixed(2)+'"',rain24<0.02,rain24>=0.02?"Wet leaves — washes off before absorbing. Wait for leaves to dry":"Dry — good absorption");
      pass("Rain Today",rainToday?fcRainLabel:"Clear",!rainToday,rainToday?"Rain forecast — needs 4-6hrs on dry leaves":"Good"); break;
    case "pesticide":
      ...
      pass("Rain",rain24.toFixed(2)+'"',rain24<0.02,rain24>=0.02?"Wet foliage reduces effectiveness — wait for leaves to dry":"Dry — good");
      pass("Rain Today",rainToday?fcRainLabel:"Clear",!rainToday,rainToday?"Rain will wash product off":"Good"); break;
    case "fert_spray":
      pass("Wind",Math.round(wind)+" mph",wind<10,wind>=10?"Drift risk":"Good");
...
function Yard({ weather, history, forecast=[] }) {
  ...
  function selectActivity(id) {
    ...
    if (!weather) return;
    setEvalResult(evalActivity(id, weather, history, forecast));
  }
  ...
  async function getAiExplanation(activity, result) {
    ...
    const fcSummary = forecast.slice(0,3).map((d,i)=>{
      const label = i===0?"Today":i===1?"Tomorrow":"Day 3";
      return `${label}: high ${Math.round(d.maxF)}F / low ${Math.round(d.minF)}F, rain ${d.rain.toFixed(2)}"`;
    }).join("; ");
...
3-day forecast: ${fcSummary}
Pressure trend: ${trend.label}

Confirm the verdict plainly and explain the single most important blocking condition. If verdict is wait or no, use the forecast and pressure trend to estimate a specific upcoming window. Be direct. One sentence per point. No em dashes.`;
    try {
  ...
  return (
    <div>
      ...
      <div style={{...css.card,marginTop:4}}>
        ...
        <input ref={productRef} type="file" accept="image/*" style={{display:"none"}} onChange={async e=>{
          const f=e.target.files?.[0]; if(!f) return;
...
function Now({ weather, locationName, loading, apiError, history, forecast, tempOffset=0 }) {
  ...
  const [showPressHist,setShowPressHist]= useState(false);
  const [showWindHist, setShowWindHist] = useState(false);
...
Wind: ${Math.round(weather.wind_speed_10m)} mph ${windLabel(weather.wind_direction_10m)} (direction from ${weather.wind_direction_source==="grid"?"regional grid — PWS obstructed by house/trees":"PWS"})
...
Write a 3-sentence field observation. Sentence 1: current conditions summary with key numbers. Sentence 2: one or two specific biological indicators for this date in NJ (bird, insect, or plant phenology). Sentence 3: 12-hour outlook based on pressure trend. Clinical and brief. No em dashes.`;
  ...
  return (
    <div>
        ...
        <div style={{...css.card,marginBottom:10}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
            {[
              ...
              {l:"Wind",v:`${Math.round(weather.wind_speed_10m)} mph ${windLabel(weather.wind_direction_10m)}`,om:weather.wind_direction_source==="grid"},
              ...
              <div key={s.l} style={{background:C.bg,borderRadius:8,padding:"8px 6px",textAlign:"center"}}>
                <div style={{fontSize:10,color:C.textDim,marginBottom:2,fontFamily:F.body,display:"flex",alignItems:"center",justifyContent:"center",gap:3}}>
                  {s.l}
                  {s.om && <span style={{fontSize:9,color:C.textDim,background:C.sep,borderRadius:4,padding:"0px 3px",lineHeight:"14px"}}>OM</span>}
                </div>
        ...
        <div style={css.card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              ...
              <InfoButton text="Rapidly falling pressure (0.09+ inHg in 6hrs) means significant weather change imminent. Rising means improving."/>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom: history.length>2 ? 10 : 0}}>
          ...
          {history.length>2 && (
            <>
              <button onClick={()=>setShowPressHist(o=>!o)} style={{background:"none",border:"none",padding:0,cursor:"pointer",display:"flex",alignItems:"center",gap:6,marginBottom: showPressHist ? 10 : 0}}>
                <span style={{fontSize:11,color:C.accent,fontFamily:F.body,fontWeight:"600",letterSpacing:0.5,textTransform:"uppercase"}}>
                  {showPressHist ? "Hide readings" : "Show readings"}
                </span>
                <span style={{fontSize:10,color:C.accent}}>{showPressHist?"▲":"▼"}</span>
              </button>
              {showPressHist && (()=>{
                ...
                return (
                  <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:10}}>
                    {recent.map((s,i)=>{
                      const val = (s.pressure*0.02953).toFixed(2);
                      const prev = recent[i+1];
                      const prevVal = prev ? prev.pressure*0.02953 : null;
                      const diff = prevVal != null ? s.pressure*0.02953 - prevVal : null;
                      const arrow = diff == null ? "" : diff > 0.005 ? "↑" : diff < -0.005 ? "↓" : "→";
                      const arrowColor = arrow==="↑" ? C.good : arrow==="↓" ? C.danger : C.textDim;
                      return (
                        <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:i<recent.length-1?`1px solid ${C.sep}`:"none"}}>
                          <span style={{fontSize:12,color:C.textDim,fontFamily:F.body,width:52}}>{fmt(s.ts)}</span>
                          <span style={{fontSize:14,color:i===0?C.text:C.textMid,fontFamily:F.num,fontWeight:i===0?"600":"400",flex:1,textAlign:"center"}}>{val}"</span>
                          <span style={{fontSize:16,color:arrowColor,width:20,textAlign:"right",fontWeight:"600"}}>{arrow}</span>
                        </div>
                      );
                    })}
                  </div>
      ...
      {weather && (
        <div style={css.card}>
          <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:10}}>
            ...
            <InfoButton text="Veering = clockwise rotation (↻ green). Backing = counter-clockwise (↺ red). Direction toward S/SW means warmer/moister air. Toward N/NW means cooler/drier."/>
          </div>

          {/* Current reading */}
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom: history.length>2 ? 10 : 0}}>
            ...
            <div>
              <div style={{fontSize:22,fontFamily:F.num,color:C.text,fontWeight:"300"}}>{windLabel(weather.wind_direction_10m)} — {Math.round(weather.wind_speed_10m)} mph</div>
              <div style={{fontSize:12,color:C.textMid,fontFamily:F.body,marginTop:2}}>
                {weather.wind_speed_10m>=15?"strong":weather.wind_speed_10m>=10?"breezy":"light"}
                {weather.wind_direction_source==="grid" && <span style={{color:C.textDim,marginLeft:6}}>· direction from grid</span>}
              </div>
            </div>
          </div>

          {/* Backing/veering alert — always show if active */}
          {shift && (
            <div style={{marginBottom:10,padding:"10px 12px",borderRadius:10,background:`${shift.color}14`,border:`1px solid ${shift.color}44`,display:"flex",alignItems:"flex-start",gap:8}}>
              <span style={{fontSize:15,color:shift.type==="veering"?C.good:C.danger,flexShrink:0}}>{shift.type==="veering"?"↻":"↺"}</span>
              <div style={{fontSize:13,color:C.text,lineHeight:1.5,fontFamily:F.body}}>{shift.label}</div>
            </div>
          )}

          {/* Direction history — collapsible */}
          {history.length>2 && (
            <>
              <button onClick={()=>setShowWindHist(o=>!o)} style={{background:"none",border:"none",padding:0,cursor:"pointer",display:"flex",alignItems:"center",gap:6,marginBottom: showWindHist ? 10 : 0}}>
                <span style={{fontSize:11,color:C.accent,fontFamily:F.body,fontWeight:"600",letterSpacing:0.5,textTransform:"uppercase"}}>
                  {showWindHist ? "Hide history" : "Show history"}
                </span>
                <span style={{fontSize:10,color:C.accent}}>{showWindHist?"▲":"▼"}</span>
              </button>
              {showWindHist && (()=>{
                const cardToDeg = {"N":0,"NNE":22.5,"NE":45,"ENE":67.5,"E":90,"ESE":112.5,"SE":135,"SSE":157.5,"S":180,"SSW":202.5,"SW":225,"WSW":247.5,"W":270,"WNW":292.5,"NW":315,"NNW":337.5};
                const recent = history.slice(-12).filter(s=>s.windDir).reverse();
                ...
                return (
                  <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:10}}>
                    {recent.map((s,i)=>{
                      const prev = recent[i+1];
                      let arrow = null, arrowColor = C.textDim;
                      if (prev && s.windDir && prev.windDir) {
                        const d1 = cardToDeg[prev.windDir], d2 = cardToDeg[s.windDir];
                        if (d1 != null && d2 != null) {
                          let diff = d2 - d1;
                          if (diff > 180) diff -= 360;
                          if (diff < -180) diff += 360;
                          if (Math.abs(diff) >= 22.5) {
                            // CW (veering) = green ↻, CCW (backing) = red ↺
                            arrow      = diff > 0 ? "↻" : "↺";
                            arrowColor = diff > 0 ? C.good : C.danger;
                          }
                        }
                      }
                      return (
                        <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:i<recent.length-1?`1px solid ${C.sep}`:"none"}}>
                          <span style={{fontSize:12,color:C.textDim,fontFamily:F.body,width:52}}>{fmt(s.ts)}</span>
                          <span style={{fontSize:14,color:i===0?C.text:C.textMid,fontFamily:F.num,fontWeight:i===0?"600":"400",flex:1,textAlign:"center"}}>
                            {s.windDir} {s.windSpeed!=null?Math.round(s.windSpeed)+" mph":""}
                          </span>
                          <span style={{fontSize:16,color:arrowColor,width:20,textAlign:"right",fontWeight:"600"}}>
                            {arrow||""}
                          </span>
                        </div>
                      );
      ...
      {/* ── 10-day forecast — only shown when frost is plausible ── */}
      {forecast && forecast.length>0 && forecast.slice(0,10).some(d => (d.minF + tempOffset) < 40) && (
        <div style={css.card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:4}}>
      ...
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",background:C.surface,borderRadius:"20px 20px 0 0",height:"88vh",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"16px 20px 40px"}}>
          ...
          <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:20,marginBottom:20}}>
            ...
            {/* ── Plant Storage Recovery ── */}
            {(()=>{
              const allPlants = loadLS("pl_plants",[]);
              if (!allPlants.length) return null;

              // Collect all current plant keys
              const currentKeys = new Set(allPlants.map(p=>"pl_pjournal_"+p.id));

              // Scan ALL localStorage for orphaned journal keys
              const orphans = [];
              try {
                for (let i=0; i<localStorage.length; i++) {
                  const k = localStorage.key(i);
                  if (k && k.startsWith("pl_pjournal_") && !currentKeys.has(k)) {
                    const data = loadLS(k,[]);
                    if (data.length > 0) orphans.push({key:k, entries:data});
                  }
                }
              } catch {}

              const totalOrphanEntries = orphans.reduce((s,o)=>s+o.entries.length,0);

              return (
                <div style={{marginTop:14,borderTop:`1px solid ${C.sep}`,paddingTop:14}}>
                  <div style={{fontSize:13,fontWeight:"600",color:C.text,fontFamily:F.body,marginBottom:6}}>
                    Plant Storage Check
                  </div>
                  ...
                  {allPlants.map(p=>{
                    const entries = loadLS("pl_pjournal_"+p.id,[]);
                    return (
                      <div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.sep}`}}>
                        <div>
                          <div style={{fontSize:13,color:C.text,fontFamily:F.body}}>{p.emoji} {p.name}</div>
                          <div style={{fontSize:10,color:C.textDim,fontFamily:F.body}}>id: {p.id}</div>
                        </div>
                        <div style={{fontSize:13,color:entries.length>0?C.accent:C.textDim,fontFamily:F.body,fontWeight:"600"}}>
                  ...
                  {/* Orphan recovery */}
                  {orphans.length > 0 && (
                    <div style={{marginTop:12,padding:"12px",background:"#FFF9F0",borderRadius:10,border:`1px solid ${C.warn}44`}}>
                      <div style={{fontSize:13,color:C.warn,fontWeight:"600",fontFamily:F.body,marginBottom:6}}>
                        ⚠ {totalOrphanEntries} orphaned entries found
                      </div>
                      <div style={{fontSize:12,color:C.textMid,fontFamily:F.body,lineHeight:1.6,marginBottom:10}}>
                        These entries were saved under old plant IDs. Tap "Recover All" to merge them into your current plants by matching names, or assign them manually.
                      </div>

                      {/* One-tap recover: merge all orphans into matching plants */}
                      <button onClick={()=>{
                        let recovered = 0;
                        orphans.forEach(o=>{
                          // Try to match orphan key to a plant by scanning global journal for plantName hints
                          // Also try merging all into the most recently active matching plant
                          allPlants.forEach(p=>{
                            const destKey = "pl_pjournal_"+p.id;
                            const existing = loadLS(destKey,[]);
                            // Only merge if this plant has no entries yet (safe default)
                            // or if we can confirm via global journal
                            const globalEntries = loadLS("pl_journal",[]);
                            const matchedGlobal = globalEntries.filter(e=>
                              e.plantName===p.name && o.entries.some(oe=>oe.id===e.id)
                            );
                            if (matchedGlobal.length > 0) {
                              const toAdd = o.entries.filter(oe=>!existing.some(ex=>ex.id===oe.id));
                              const merged = [...toAdd,...existing].sort((a,b)=>b.id-a.id);
                              saveLS(destKey, merged);
                              recovered += toAdd.length;
                            }
                          });
                        });
                        if (recovered > 0) {
                          alert(`Recovered ${recovered} entries. Close Settings and reopen the app.`);
                        } else {
                          // Fallback: show manual assignment
                          alert("Could not auto-match entries. Use Manual Recovery below to assign orphaned entries to a plant.");
                        }
                      }} style={{...css.btn,width:"100%",textAlign:"center",padding:"10px",fontSize:13,marginBottom:8}}>
                        Recover All (Auto-match)
                      </button>

                      {/* Manual recovery: list orphan keys with assign buttons */}
                      <div style={{fontSize:11,color:C.textMid,fontFamily:F.body,marginBottom:6,fontWeight:"600",letterSpacing:0.5,textTransform:"uppercase"}}>Manual Assignment</div>
                      {orphans.map(o=>(
                        <div key={o.key} style={{marginBottom:8,padding:"8px",background:C.surface,borderRadius:8}}>
                          <div style={{fontSize:11,color:C.textDim,fontFamily:F.body,marginBottom:4}}>
                            {o.key.replace("pl_pjournal_","")} · {o.entries.length} entries
                          </div>
                          <div style={{fontSize:12,color:C.textMid,fontFamily:F.body,marginBottom:6}}>
                            Latest: "{o.entries[0]?.notes||o.entries[0]?.note||"(photo only)"}"
                          </div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                            {allPlants.map(p=>(
                              <button key={p.id} onClick={()=>{
                                ...
                                const merged = [...toAdd,...existing].sort((a,b)=>b.id-a.id);
                                saveLS(destKey, merged);
                                saveCritical(destKey, merged);
                                // Remove orphan key
                                localStorage.removeItem(o.key);
                                alert(`Moved ${toAdd.length} entries to ${p.name}. Close and reopen Settings to refresh.`);
                              }} style={{...css.btnSoft,padding:"5px 10px",fontSize:11}}>
                                → {p.emoji} {p.name}
                              </button>
                            ))}
                          </div>
                  ...
                  {orphans.length===0 && (
                    <div style={{marginTop:8,fontSize:12,color:C.accent,fontFamily:F.body}}>✓ All journal entries are correctly assigned.</div>
                  )}

                  {/* Storage usage and cleanup */}
                  {(()=>{
                    let total = 0;
                    try { for (let i=0;i<localStorage.length;i++) { const k=localStorage.key(i); if(k) total += (localStorage.getItem(k)||"").length; } } catch {}
                    const kb = Math.round(total/1024);
                    const pct = Math.round(kb/5120*100);
                    return (
                      <div style={{marginTop:12,padding:"10px 12px",background:kb>4000?"#FFF2F2":kb>3000?"#FFF9F0":C.accentDim,borderRadius:10,border:`1px solid ${kb>4000?C.danger:kb>3000?C.warn:C.accent}44`}}>
                        <div style={{fontSize:12,fontWeight:"600",color:kb>4000?C.danger:kb>3000?C.warn:C.accent,fontFamily:F.body,marginBottom:6}}>
                          Storage: ~{kb}KB used of ~5120KB ({pct}%)
                          {kb>4000?" ⚠ NEARLY FULL — entries can't save!":kb>3000?" ⚠ Getting full":""}
                        </div>
                        <button onClick={()=>{
                          // 1. Remove all orphaned keys
                          let freed = 0;
                          const currentPlantKeys = new Set(loadLS("pl_plants",[]).map(p=>"pl_pjournal_"+p.id));
                          const toDelete = [];
                          try { for(let i=0;i<localStorage.length;i++){ const k=localStorage.key(i); if(k&&k.startsWith("pl_pjournal_")&&!currentPlantKeys.has(k)) toDelete.push(k); } } catch {}
                          toDelete.forEach(k=>{ freed += (localStorage.getItem(k)||"").length; localStorage.removeItem(k); });
                          // 2. Strip photos from history (biggest culprit)
                          const hist = loadLS("pl_history",[]);
                          const histStripped = hist.map(s=>({...s,photo:undefined}));
                          saveLS("pl_history", histStripped);
                          // 3. Strip photos from current plant journals (photos now in IDB only)
                          loadLS("pl_plants",[]).forEach(p=>{
                            const jkey = "pl_pjournal_"+p.id;
                            const entries = loadLS(jkey,[]);
                            if (entries.length) {
                              // Move photos to IDB, strip from localStorage
                              entries.forEach(e=>{ if(e.photo) { idbSet("photo_"+e.id, e.photo); e.photo=null; } });
                              saveLS(jkey, entries);
                            }
                          });
                          const freedKB = Math.round(freed/1024);
                          alert(`Freed ~${freedKB}KB from orphaned keys. Photos moved to device database. Close and reopen the app.`);
                        }} style={{...css.btn,width:"100%",textAlign:"center",padding:"9px",fontSize:12}}>
...
export default function App() {
  ...
  const [chatFocusPlant, setChatFocusPlant] = useState(null);
  ...
  useEffect(()=>{
    const iv = setInterval(()=>{ if (localStorage.getItem("pl_awn_key")) fetchWeather(); }, 10*60*1000);
    return ()=>clearInterval(iv);
  },[]);

  // On startup, check if localStorage was cleared and restore critical data from IndexedDB
  useEffect(()=>{
    (async () => {
      const plants = loadLS("pl_plants", null);
      if (plants !== null) {
        // localStorage intact — mirror to IDB as backup
        await idbSet("pl_plants", JSON.stringify(plants));
        // Also mirror any plant journals that exist
        plants.forEach(async p => {
          const j = loadLS("pl_pjournal_"+p.id, null);
          if (j !== null) await idbSet("pl_pjournal_"+p.id, JSON.stringify(j));
        });
      } else {
        // localStorage was cleared — restore from IndexedDB
        const idbPlants = await idbGet("pl_plants");
        if (idbPlants) {
          const parsed = JSON.parse(idbPlants);
          saveLS("pl_plants", parsed);
          console.log("Restored pl_plants from IndexedDB");
          // Restore all plant journals
          for (const p of parsed) {
            const idbJournal = await idbGet("pl_pjournal_"+p.id);
            if (idbJournal) {
              saveLS("pl_pjournal_"+p.id, JSON.parse(idbJournal));
              console.log("Restored journal for", p.name);
            }
          }
          // Restore global journal
          const idbGlobal = await idbGet("pl_journal");
          if (idbGlobal) saveLS("pl_journal", JSON.parse(idbGlobal));
    ...
    try {
      if (awnKey && awnAppKey && awnMac) {
        try {
          ...
          // ── Fetch 24hr AWN history ────────────────────────────────────────
          // Delayed 3s to avoid rate-limiting the current conditions call.
          // Only fetches once per hour — history doesn't need to be fresher.
          (async () => {
            const lastHistFetch = parseInt(localStorage.getItem("pl_hist_last")||"0");
            const histAge = Date.now() - lastHistFetch;
            if (histAge < 60*60*1000) return; // skip if fetched within last hour
            await new Promise(r => setTimeout(r, 3000)); // 3s delay
            try {
              const histUrl = `https://rt.ambientweather.net/v1/devices/${encodeURIComponent(awnMac)}?apiKey=${awnKey}&applicationKey=${awnAppKey}&limit=288`;
              const histRes = await fetch(histUrl);
              if (!histRes.ok) throw new Error("AWN history "+histRes.status);
              const histData = await histRes.json();
              if (!Array.isArray(histData) || !histData.length) throw new Error("No history data");

              // Convert each AWN record to a history snapshot
              const newSnaps = histData
                .map(rec => {
                  const r = rec.lastData || rec;
                  if (!r.tempf || !r.dateutc) return null;
                  // AWN dateutc is ms timestamp or ISO string
                  const ts = typeof r.dateutc === "number" ? r.dateutc : new Date(r.dateutc).getTime();
                  if (isNaN(ts)) return null;
                  return {
                    ts,
                    temp:          r.tempf,
                    humidity:      r.humidity     || 0,
                    pressure:      r.baromrelin ? r.baromrelin/0.02953 : (r.baromabsin ? r.baromabsin/0.02953 : 1013),
                    windSpeed:     r.windspeedmph || 0,
                    windDir:       windLabel(r.winddir || 0),
                    precipitation: r.hourlyrainin || 0,
                    _source:       "awn_history",
                  };
                })
                .filter(Boolean)
              ...
              // Merge: keep existing app-recorded snaps, fill gaps with AWN history
              // Deduplicate by rounding to nearest 5-min bucket
              const bucket = ts => Math.round(ts / (5*60*1000));
              const existing = loadLS("pl_history", []);
              const existingBuckets = new Set(existing.map(s => bucket(s.ts)));
              const toAdd = newSnaps.filter(s => !existingBuckets.has(bucket(s.ts)));
              if (!toAdd.length) return;

              const merged = [...existing, ...toAdd]
                .sort((a,b)=>a.ts-b.ts)
                .slice(-1000); // keep max 1000 snaps

              setHistory(merged);
              saveLS("pl_history", merged);
              localStorage.setItem("pl_hist_last", String(Date.now())); // record successful fetch time
            } catch(e) {
              // Silent — history is a nice-to-have, don't surface errors
              console.log("AWN history fetch:", e.message);
            }
          })();
          try {
            const pos = await new Promise((res,rej)=>navigator.geolocation.getCurrentPosition(res,rej,{timeout:5000}));
            // Fetch forecast + current grid temp + grid wind direction in one call
            const fc = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${pos.coords.latitude}&longitude=${pos.coords.longitude}&current=temperature_2m,wind_direction_10m&daily=temperature_2m_min,temperature_2m_max,precipitation_sum&temperature_unit=fahrenheit&precipitation_unit=inch&timezone=auto&forecast_days=10`);
            ...
            // Overlay grid wind direction onto PWS weather object
            // PWS wind direction is affected by house/trees — grid direction is more representative of synoptic flow
            if (fd.current?.wind_direction_10m != null) {
              wx.wind_direction_10m = fd.current.wind_direction_10m;
              wx.wind_direction_source = "grid";
              setWeather({...wx});
            }
            ...
            async p => {
              try {
                const [wr,gr] = await Promise.all([
                  fetch(`https://api.open-meteo.com/v1/forecast?latitude=${p.coords.latitude}&longitude=${p.coords.longitude}&current=temperature_2m,relative_humidity_2m,surface_pressure,wind_speed_10m,wind_direction_10m,precipitation&daily=temperature_2m_min,temperature_2m_max,precipitation_sum&temperature_unit=fahrenheit&wind_speed_unit=mph&precipitation_unit=inch&timezone=auto&forecast_days=10`),
  ...
  return (
    <div style={{fontFamily:F.body,background:C.bg,height:"100dvh",color:C.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* ── Garden Chat — rendered at App level so it's above tab bar ── */}
      {showChat && (
        <GardenChat
          weather={weather}
          history={history}
          plants={loadLS("pl_plants",[])}
          tempOffset={tempOffset}
          forecast={forecast}
          focusPlant={chatFocusPlant}
          onClose={()=>{ setShowChat(false); setChatFocusPlant(null); }}
        />
      )}
      ...
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"14px 14px 100px"}}>
        ...
        {tab==="garden" && <Garden weather={weather} history={history} tempOffset={tempOffset} forecast={forecast} onOpenChat={(plant=null)=>{ setChatFocusPlant(plant); setShowChat(true); }}/>}
        {tab==="yard"   && <Yard   weather={weather} history={history} forecast={forecast}/>}
      </div>
