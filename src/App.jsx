...
function wateringAdv(wx, hist, offset=0) {
  // PWS rain24h is the authoritative source when present and >0.
  // Only sum history if PWS shows zero (avoids stale history accumulation).
  const pwsRain  = wx.rain24h || 0;
  const histRain = pwsRain > 0 ? 0 : hist.slice(-48).reduce((s,r)=>s+(r.precipitation||0),0);
  const rain     = pwsRain > 0 ? pwsRain : histRain;
  const rainSrc  = pwsRain > 0 ? "PWS gauge" : "station history";
