...
// ══════════════════════════════════════════════════════════════════════════════
function GardenChat({ weather, history, plants, tempOffset=0, forecast=[], onClose }) {
  ...
  function buildSystemPrompt() {
    ...
    const plantContext = plants.length ? plants.map(p => {
      ...
      const notes = entries.length
        ? entries.map(e=>`  ${e.date}: ${e.notes||e.note||"(photo only)"}`).join("\n")
        : "  No journal entries yet.";
      return `${p.emoji} ${p.name} (${p.location}${p.planted?", planted "+p.planted:""}):\n${notes}`;
    }).join("\n\n") : "No plants registered yet.";

    return `You are a knowledgeable garden and plant diagnostician embedded in the Plot app. You have full access to the user's backyard conditions, garden advisor readings, 10-day forecast, and plant journal history. Be specific, practical, and terse. Reference plant names, journal observations, advisor readings, and upcoming weather directly. Ask clarifying questions if needed. No em dashes.
            ...
            <div>
              <div style={{textAlign:"center",padding:"16px 0 14px"}}>
                ...
                <div style={{fontSize:14,color:C.textMid,fontFamily:F.body,lineHeight:1.6}}>
                  I know your plants, conditions, and journal notes. Ask me anything.
                </div>
              </div>
  ...
  return (
    <div style={{fontFamily:F.body,background:C.bg,height:"100dvh",color:C.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"14px 14px 100px"}}>
        ...
        {tab==="garden" && <Garden weather={weather} history={history} tempOffset={tempOffset} forecast={forecast} onOpenChat={()=>setShowChat(true)}/>}
