...
function Garden({ weather, history, tempOffset=0 }) {
  ...
  const soil   = adjTemp ? soilEst(adjTemp, month) : null;
  const frost  = adjTemp ? frostRisk(adjTemp, weather.relative_humidity_2m, weather.wind_speed_10m) : null;
  const water  = adjTemp ? wateringAdv({...weather, temperature_2m: adjTemp}, history, offset) : null;
  const fung   = adjTemp ? fungalRisk({...weather, temperature_2m: adjTemp}, history, offset) : null;
      ...
      <div style={{...css.card, borderColor:`${C.accent}33`, marginBottom:10}}>
        ...
        {genJournal.length>0 && (
          <div style={{marginTop: showGenJournal ? 8 : 0}}>
            ...
            {genJournal.length>10 && (
              <div style={{textAlign:"center",fontSize:13,color:C.textDim,fontFamily:F.body,padding:"8px 0"}}>
                {genJournal.length-10} older entries saved
              </div>
            )}
          </div>
        )}
...
export default function App() {
  ...
  const [tempOffset, setTempOffset] = useState(()=>parseFloat(localStorage.getItem("pl_temp_off")||"0"));
  ...
  return (
    <div style={{fontFamily:F.body,background:C.bg,height:"100dvh",color:C.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      {showSettings && <Settings onClose={()=>{ setShowSettings(false); fetchWeather(); }} tempOffset={tempOffset} offsetPairs={offsetPairs} onManualOffset={(v)=>{ setTempOffset(v); localStorage.setItem("pl_temp_off",String(v)); }}/>}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{textAlign:"right"}}>
              ...
              {tempOffset!==0 && weather?.source==="pws" && <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",marginTop:1,fontFamily:F.body}}>{tempOffset>0?"+":""}{tempOffset.toFixed(1)}F yard adj</div>}
            </div>
