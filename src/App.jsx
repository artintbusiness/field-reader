...
const APP_VERSION = "2.1.1"; // Update this on every deploy to force cache refresh
// ─── DESIGN TOKENS ────────────────────────────────────────────────────────────
...
function exportBackup() {
  ...
  plants.forEach(p => {
    ...
    if (entries.length) plantJournals["pl_pjournal_" + p.id] = entries;
  });
  const backup = {
    version: APP_VERSION,
  ...
  a.href = url;
  a.download = "plot-backup-" + new Date().toLocaleDateString("en-US",{month:"2-digit",day:"2-digit",year:"numeric"}).replace(/\//g,"-") + ".json";
  a.click();
  URL.revokeObjectURL(url);
}
...
function importBackup(file, onDone) {
  ...
  reader.onload = e => {
    try {
      ...
      if (data.plantJournals) {
        Object.entries(data.plantJournals).forEach(([key, val]) => saveLS(key, val));
      }
      onDone(true, data.pl_plants?.length || 0, data.pl_journal?.length || 0);
    } catch { onDone(false, 0, 0); }
  };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#00000066",zIndex:500,display:"flex",alignItems:"flex-end"}}>
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",background:C.surface,borderRadius:"20px 20px 0 0",padding:"24px 20px 40px"}}>
        ...
        <div style={{marginTop:20,paddingTop:20,borderTop:`1px solid ${C.sep}`}}>
          <div style={{fontSize:14,fontWeight:"600",color:C.text,fontFamily:F.body,marginBottom:6}}>Data Backup</div>
          <div style={{fontSize:13,color:C.textMid,fontFamily:F.body,lineHeight:1.6,marginBottom:14}}>
            Export your plants and journal entries before updates. Import to restore after.
          </div>
          <div style={{display:"flex",gap:10,marginBottom:10}}>
            <button onClick={exportBackup} style={{...css.btn,flex:1,textAlign:"center",padding:"12px",fontSize:14}}>Export Backup</button>
            <button onClick={()=>restoreRef.current?.click()} style={{...css.btnSoft,flex:1,textAlign:"center",padding:"12px",fontSize:14}}>Restore Backup</button>
          </div>
          ...
          <input ref={restoreRef} type="file" accept=".json" style={{display:"none"}} onChange={e=>{
            const f=e.target.files?.[0]; if(!f) return;
            importBackup(f,(ok,plants,entries)=>{
              setRestoreMsg(ok
                ? {ok:true,text:"Restored "+plants+" plants and "+entries+" journal entries. Close Settings and reload."}
                : {ok:false,text:"Could not read file. Make sure it is a Plot backup JSON."});
            });
          }}/>
        </div>
      </div>
...
// ─── PLANT ENTRY CARD ─────────────────────────────────────────────────────────
function PlantEntryCard({ entry: e, onDelete, onUpdate }) {
    ...
    return (
      <div style={{...css.card,borderColor:`${C.accent}44`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={()=>setEditing(false)} style={{background:"none",border:"none",color:C.textDim,cursor:"pointer",fontSize:16,fontFamily:F.body}}>Cancel</button>
        </div>
        {e.photo&&<img src={e.photo} style={{width:"100%",borderRadius:10,marginBottom:12,maxHeight:160,objectFit:"cover"}}/>}
        <textarea value={draftNote} onChange={ev=>setDraftNote(ev.target.value)}
          rows={5} style={{...css.input,resize:"none",fontSize:15,lineHeight:1.6,marginBottom:12}} placeholder="Notes..."/>
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>{
            onUpdate({...e,note:draftNote});
            const mainJournal = loadLS("pl_journal",[]);
            const updatedMain = mainJournal.map(x=>x.id===e.id?{...x,note:draftNote,notes:draftNote}:x);
            saveLS("pl_journal",updatedMain);
            setEditing(false);
          }} style={{...css.btn,flex:1,textAlign:"center",padding:"13px"}}>Save</button>
          <button onClick={onDelete} style={{background:"transparent",border:`1px solid ${C.danger}22`,color:C.danger,padding:"13px 18px",borderRadius:24,fontSize:14,cursor:"pointer",fontFamily:F.body}}>Delete</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={css.card}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
        <div style={{fontSize:11,color:C.accent,fontFamily:F.body,fontWeight:"600"}}>{e.date} - {e.time}</div>
        <div style={{display:"flex",gap:4}}>
          <button onClick={()=>setEditing(true)} style={{background:"none",border:`1px solid ${C.sep}`,color:C.textMid,cursor:"pointer",fontSize:11,borderRadius:8,padding:"3px 8px",fontFamily:F.body}}>Edit</button>
          <button onClick={onDelete} style={{background:"none",border:"none",color:C.textDim,cursor:"pointer",fontSize:16,lineHeight:1}}>x</button>
        </div>
      </div>
      {e.photo&&<img src={e.photo} style={{width:"100%",borderRadius:10,marginBottom:8,maxHeight:180,objectFit:"cover"}}/>}
      {e.note&&<div style={{fontSize:13,color:C.text,lineHeight:1.6,fontFamily:F.body}}>{e.note}</div>}
    </div>
  );
}
...
function PlantJournal({ plant, onClose }) {
  ...
  function updateEntry(updated) {
    // Update plant journal
    const u = entries.map(x=>x.id===updated.id?updated:x);
    setEntries(u); saveLS(key, u);
    // Sync to main journal
    const main = loadLS("pl_journal",[]);
    const updatedMain = main.map(x=>x.id===updated.id?{...x,note:updated.note,notes:updated.note}:x);
    saveLS("pl_journal", updatedMain);
  }
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#00000066",zIndex:500,display:"flex",alignItems:"flex-end"}}>
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",background:C.surface,borderRadius:"20px 20px 0 0",maxHeight:"92vh",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"20px 20px 12px",borderBottom:`1px solid ${C.sep}`,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={onClose} style={{background:"none",border:"none",color:C.textMid,fontSize:22,cursor:"pointer"}}>x</button>
          </div>
        </div>

        <div style={{overflowY:"auto",flex:1,padding:"16px 20px 40px"}}>
          {/* How to add notes hint */}
          <div style={{...css.card,background:C.accentDim,borderColor:`${C.accent}33`,marginBottom:16,padding:"12px 14px"}}>
            <div style={{fontSize:13,color:C.accent,fontFamily:F.body,fontWeight:"600",marginBottom:3}}>📝 Adding notes</div>
            <div style={{fontSize:13,color:C.textMid,fontFamily:F.body,lineHeight:1.6}}>
              Go to the <strong>Journal tab</strong> and mention <strong>{plant.name}</strong> in your entry. It will be automatically filed here.
            </div>
          </div>

          {entries.length === 0 && (
            <div style={{textAlign:"center",padding:"32px 16px",color:C.textDim,fontSize:13,fontFamily:F.body,lineHeight:1.8}}>
              <div style={{fontSize:28,marginBottom:8}}>{plant.emoji}</div>
              No journal entries yet.<br/>Mention {plant.name} in the Journal tab to start.
            </div>
          )}
    ...
    <div style={{position:"relative",display:"inline-block"}}>
      <button
        ...
        style={{
          ...
          fontSize:14,color:C.textDim,padding:"2px 4px",
      ...
      {show&&(
        <>
          ...
          <div style={{
            position:"fixed",right:16,bottom:100,zIndex:999,
            background:C.surface,border:`1px solid ${C.sep}`,
...
function Garden({ weather, history }) {
  ...
  function addPlant() {
    if (!np.name) return;
    const capitalized = {...np, name: np.name.charAt(0).toUpperCase() + np.name.slice(1), id:Date.now()};
    const u=[...plants, capitalized]; setPlants(u); saveLS("pl_plants",u);
    ...
    <div>
      ...
      <div style={{...css.card,borderColor:`${water.color}44`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Watering Advisor</span>
          <InfoButton text="Combines 24hr rainfall, adjusted humidity, temperature, and wind into a daily recommendation. Based on your PWS data when connected. Use the finger test to confirm before watering."/>
        </div>
      ...
      <div style={{...css.card,borderColor:`${fung.color}33`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Fungal Disease Risk</span>
          <InfoButton text="Tracks warm, wet, humid conditions. Elevated risk when humidity stays above 80%, temps 60-85F, and recent rain exceeds 0.25 inches. The exact conditions that cause blight and mildew."/>
        </div>
      ...
      <div style={css.card}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Soil Temperature (estimated)</span>
          <InfoButton text="Estimated from air temp and season. Soil lags air by several days. The real trigger for germination and transplanting - more reliable than calendar date."/>
        </div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <InfoButton text="Register your plants here. Each gets its own journal log. The AI Garden Advisor reads your plant notes when generating advice. Tap Journal on any plant card to add observations."/>
      ...
      <div style={{...css.card,background:"linear-gradient(135deg,#e8f5ee,#f0faf2)",borderColor:`${C.accent}33`,marginTop:4}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Garden Advisor</span>
          <InfoButton text="AI-powered advice using your current conditions and plant journal history. References the pressure trend to suggest upcoming windows when conditions are not right today."/>
        </div>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button onClick={()=>exportGardenLog(plants, history)} style={{...css.btnSoft,padding:"7px 12px",fontSize:12}}>Export PDF</button>
            <button onClick={getAdvice} style={{...css.btn,padding:"7px 14px",fontSize:12}}>{advLoad?"Thinking...":"Get Advice"}</button>
          </div>
        </div>
        {advice
...
function Yard({ weather, history }) {
  ...
  function selectActivity(id) {
    setSelected(id);
    setAiExplain("");
    if (!weather) return <div style={{textAlign:"center",padding:48,color:C.textMid,fontFamily:F.body,fontSize:14}}>Loading...</div>;
...
// ─── NOW TAB ──────────────────────────────────────────────────────────────────
function Now({ weather, locationName, loading, apiError, history, forecast }) {
  ...
  return (
    <div>
      ...
      {weather&&!weather.source==="pws"&&(
        <div style={css.card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Barometric Pressure</span>
          <InfoButton text="The most predictive single weather signal. Rapidly falling pressure (3+ hPa in 6 hours) means a significant weather change is imminent. Rising pressure means improving conditions."/>
        </div>
...
{weather&&(
          <div style={css.card}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Wind</span>
          <InfoButton text="Current wind speed and direction from your station. Direction tells you what air mass is moving in - northwest winds bring cold dry air, southwest often precedes warm fronts."/>
        </div>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div>
                {weather&&<div style={{fontSize:22,fontFamily:F.display,color:C.text}}>{windLabel(weather.wind_direction_10m)} - {Math.round(weather.wind_speed_10m)} mph</div>}
                {weather&&<div style={{fontSize:12,color:C.textMid,fontFamily:F.body,marginTop:3}}>{windLabel(weather.wind_direction_10m)} at {Math.round(weather.wind_speed_10m)} mph{weather.wind_speed_10m>=15?" - strong":weather.wind_speed_10m>=10?" - breezy":""}</div>}
              </div>
            </div>
          </div>
)}
          ...
          {history.length>2&&(
            <div style={css.card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Trends</span>
          <InfoButton text="Pressure, temperature, humidity and wind over recent readings. Useful for spotting patterns - a multi-day pressure decline is more significant than a single low reading."/>
        </div>
          ...
          <div style={{...css.card,background:"linear-gradient(135deg,#e8f5ee,#f0faf2)",borderColor:`${C.accent}33`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Naturalist's Reading</span>
          <InfoButton text="A brief technical field observation generated from your current conditions. Includes likely biological indicators for this time of year and a 12-hour pressure-based outlook."/>
        </div>
          ...
          {/* 10-Day Frost Outlook */}
          {forecast && forecast.length > 0 && (
            <div style={css.card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <span style={{...css.label,marginBottom:0}}>10-Day Frost Outlook</span>
                <InfoButton text="Daily overnight low forecast for the next 10 days. Frost risk flagged when overnight lows approach 32F. Uses Open-Meteo forecast model."/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:4,marginBottom:8}}>
                {forecast.slice(0,10).map((day,i)=>{
                  const fr = frostRisk(day.minF, 70, 5);
                  const isToday = i===0;
                  return (
                    <div key={day.date} style={{
                      textAlign:"center",padding:"8px 4px",borderRadius:10,
                      background:fr.level!=="safe"?`${fr.color}18`:C.bg,
                      border:`1px solid ${fr.level!=="safe"?fr.color:C.sep}`,
                    }}>
                      <div style={{fontSize:9,color:C.textDim,fontFamily:F.body,marginBottom:2}}>
                        {isToday?"Today":new Date(day.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short"})}
                      </div>
                      <div style={{fontSize:12,fontWeight:"600",color:fr.level!=="safe"?fr.color:C.text,fontFamily:F.body}}>
                        {Math.round(day.minF)}F
                      </div>
                      <div style={{fontSize:10,color:C.textDim,fontFamily:F.body}}>
                        {Math.round(day.maxF)}F
                      </div>
                      {fr.level!=="safe"&&<div style={{fontSize:9,color:fr.color,marginTop:2}}>FROST</div>}
                      {day.rain>0.1&&<div style={{fontSize:9,color:"#60A5FA",marginTop:1}}>{day.rain.toFixed(1)}"</div>}
                    </div>
                  );
                })}
              </div>
              <div style={{fontSize:11,color:C.textMid,fontFamily:F.body,marginTop:2}}>Top row = overnight low · Bottom = daytime high</div>
              {forecast.slice(0,10).some(d=>frostRisk(d.minF,70,5).level!=="safe") && (
                <div style={{marginTop:10,padding:"8px 12px",background:"#FFF2F2",borderRadius:10,border:"1px solid #FFD0D0"}}>
                  <div style={{fontSize:12,color:C.danger,fontFamily:F.body,fontWeight:"600"}}>Frost risk detected in the next 10 days</div>
                </div>
              )}
            </div>
          )}
      ...
      <div style={css.card}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
          {pressData.map(d=>(
            <div key={d.day} style={{textAlign:"center",flex:1}}>
              <div style={{fontSize:10,color:C.textDim,fontFamily:F.body}}>{d.day.split(" ")[1]}</div>
              <div style={{fontSize:10,color:"#2D6A4F",fontFamily:F.body}}>{(d.val*0.02953).toFixed(2)}"</div>
            </div>
          ))}
        </div>
      </div>
...
function JournalDashboard({ history, journal }) {
  ...
  return (
    <div style={{marginBottom:4}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{...css.label,marginBottom:0}}>Last 3 Days</span>
          <InfoButton text="Quick weather summary for the past 3 days. Green dot means you logged an observation that day."/>
        </div>
...
// ─── JOURNAL ENTRY CARD ───────────────────────────────────────────────────────
function JournalEntryCard({ entry: e, onDelete, onUpdate }) {
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState({ clouds: e.clouds||"", signs: e.signs||"", notes: e.notes||"" });
  ...
  if (editing) {
    return (
      <div style={{...css.card, borderColor:`${C.accent}44`}}>
        ...
        <div style={{fontSize:11,color:C.textMid,marginBottom:4,fontFamily:F.body}}>Clouds</div>
        <input value={draft.clouds} onChange={ev=>setDraft(d=>({...d,clouds:ev.target.value}))}
          placeholder="Cloud observations..." style={css.input}/>
        <div style={{fontSize:11,color:C.textMid,marginBottom:4,fontFamily:F.body}}>Signs</div>
        <input value={draft.signs} onChange={ev=>setDraft(d=>({...d,signs:ev.target.value}))}
          placeholder="Nature signs..." style={css.input}/>
        <div style={{fontSize:11,color:C.textMid,marginBottom:4,fontFamily:F.body}}>Notes</div>
        <textarea value={draft.notes} onChange={ev=>setDraft(d=>({...d,notes:ev.target.value}))}
          rows={3} style={{...css.input,resize:"vertical"}} placeholder="Observations..."/>
        <div style={{display:"flex",gap:10}}>
          <button onClick={saveEdit} style={{...css.btn,flex:1,textAlign:"center",padding:"10px"}}>Save</button>
          <button onClick={onDelete} style={{flex:1,background:"transparent",border:`1px solid ${C.danger}22`,color:C.danger,padding:"10px",borderRadius:24,fontSize:14,cursor:"pointer",fontFamily:F.body}}>Delete</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={css.card}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
        <div style={{flex:1}}>
          <div style={{fontSize:12,color:C.accent,fontFamily:F.body,fontWeight:"600"}}>{e.date} - {e.time}</div>
          {e.weather&&<div style={{fontSize:11,color:C.textDim,marginTop:2,fontFamily:F.body}}>{e.weather}</div>}
        </div>
        <div style={{display:"flex",gap:4,alignItems:"center",flexShrink:0,marginLeft:8}}>
          <button onClick={()=>setEditing(true)} style={{background:"none",border:`1px solid ${C.sep}`,color:C.textMid,cursor:"pointer",fontSize:12,borderRadius:8,padding:"4px 10px",fontFamily:F.body}}>Edit</button>
          <button onClick={onDelete} style={{background:"none",border:"none",color:C.textDim,cursor:"pointer",fontSize:18,lineHeight:1}}>x</button>
        </div>
      </div>
      ...
      {e.notes&&<div style={{fontSize:14,color:C.text,lineHeight:1.6,marginTop:6,fontFamily:F.body}}>{e.notes}</div>}
    </div>
  );
}
...
function Journal({ weather, history }) {
  ...
  // Smart plant routing - AI detects plant mentions and routes to plant journals
  const [pendingEntry, setPendingEntry] = useState(null);
  const [suggestedPlant, setSuggestedPlant] = useState(null);
  const [showPlantConfirm, setShowPlantConfirm] = useState(false);

  async function detectAndRoute(entryText, entryObj) {
    const apiKey = localStorage.getItem("pl_anthropic_key") || "";
    if (!apiKey || !entryText) return;

    const currentPlants = loadLS("pl_plants", []);
    const plantNames = currentPlants.map(p => p.name).join(", ");
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body:JSON.stringify({
          ...
          max_tokens:150,
          messages:[{role:"user",content:
            "Extract any plant names mentioned in this garden journal entry. " +
            "Known plants: " + (plantNames || "none") + ". " +
            "Return ONLY valid JSON, no markdown, no backticks. Format: {mentioned:[plant names], new:new plant name or null} " +
            "Entry: " + entryText
          }]
        })
      });
      ...
      if (parsed.mentioned && parsed.mentioned.length > 0) {
        // Route to existing plants
        parsed.mentioned.forEach(name => {
          const plant = currentPlants.find(p => p.name.toLowerCase() === name.toLowerCase());
          if (plant) {
            const jkey = "pl_pjournal_" + plant.id;
            const existing = loadLS(jkey, []);
            saveLS(jkey, [{...entryObj, note: entryObj.notes || entryObj.note || ""}, ...existing]);
          }
        });

        // Handle new plant
        if (parsed.new) {
          setPendingEntry({...entryObj, detectedPlant: parsed.new});
          setSuggestedPlant(parsed.new);
          setShowPlantConfirm(true);
          return;
        }
      }
    } catch(e) { /* silently fail - entry already saved to main journal */ }
  }
  ...
  function addEntry() {
    ...
    const e = {
      ...
      weather:weather?(Math.round(weather.temperature_2m)+"F  "+weather.relative_humidity_2m+"% RH  "+hpa2hg(weather.surface_pressure)+"\" "+windLabel(weather.wind_direction_10m)+" "+Math.round(weather.wind_speed_10m)+"mph"+(weather.source==="pws"?"  PWS":"")):
      "",
      clouds:entry.clouds, signs, notes:capFirst(entry.notes), note:capFirst(entry.notes), photo:entry.photo,
    };
    const u = [e,...journal]; setJournal(u); saveLS("pl_journal",u);

    // Try to route to plant journals
    const fullText = [entry.notes, entry.clouds, signs].filter(Boolean).join(" ");
    if (fullText) detectAndRoute(fullText, e);
  ...
  return (
    <div>
      ...
      {showPlantConfirm && suggestedPlant && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 20px"}}>
          <div style={{background:C.surface,borderRadius:20,padding:24,maxWidth:320,width:"100%",boxShadow:"0 8px 32px rgba(0,0,0,0.2)"}}>
            <div style={{fontSize:28,marginBottom:8,textAlign:"center"}}>🌱</div>
            <div style={{fontSize:16,fontWeight:"700",color:C.text,fontFamily:F.body,marginBottom:8,textAlign:"center"}}>New Plant Detected</div>
            <div style={{fontSize:14,color:C.textMid,fontFamily:F.body,lineHeight:1.6,marginBottom:20,textAlign:"center"}}>
              Your note mentions <strong style={{color:C.accent}}>{suggestedPlant}</strong>. Add it to your plant list?
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{
                const capName2 = suggestedPlant.charAt(0).toUpperCase() + suggestedPlant.slice(1);
                const today2 = new Date().toISOString().split("T")[0];
                const entryText2 = pendingEntry?.notes || pendingEntry?.note || "";
                const locMatch2 = entryText2.match(/(?:in|at|on|from)\s+(?:the\s+)?([a-zA-Z\s]+(?:bed|pot|garden|patio|yard|container|box|planter|indoors?|outdoors?))/i);
                const autoLoc2 = locMatch2 ? locMatch2[1].trim() : "garden";
                const newPlant={id:Date.now(),name:capName2,emoji:"🌱",location:autoLoc2,planted:today2};
                const existing=loadLS("pl_plants",[]);
                saveLS("pl_plants",[...existing,newPlant]);
                if(pendingEntry) saveLS("pl_pjournal_"+newPlant.id,[{...pendingEntry}]);
                setShowPlantConfirm(false);setSuggestedPlant(null);setPendingEntry(null);
              }} style={{...css.btn,flex:1,textAlign:"center",padding:"11px"}}>Add Plant</button>
              <button onClick={()=>{setShowPlantConfirm(false);setSuggestedPlant(null);setPendingEntry(null);}} style={{flex:1,background:"transparent",border:`1px solid ${C.sep}`,color:C.textMid,padding:"11px",borderRadius:24,fontSize:14,cursor:"pointer",fontFamily:F.body}}>Skip</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,marginTop:8}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <InfoButton text="Your field log. Weather conditions attached automatically. Voice entries AI-cleaned. Plant mentions automatically routed to that plant journal."/>
        </div>
      ...
      {showForm&&(
        <div style={{...css.card,borderColor:`${C.accent}44`,marginBottom:16}}>
          <div style={{marginBottom:14}}>
            <div style={{fontSize:9,color:C.accent,letterSpacing:2,marginBottom:8,fontFamily:F.body,textTransform:"uppercase"}}>Voice Entry</div>
            <button onClick={dictating?stopDictation:startDictation} style={{...css.btn,width:"100%",textAlign:"center",padding:"12px",background:dictating?"#FFF0F0":C.accentDim,borderColor:dictating?C.danger:C.accent+"55",color:dictating?C.danger:C.accent}}>
              {dictating?"Stop Recording":"Tap to Dictate"}
            </button>
            {dictating&&rawDic&&<div style={{fontSize:12,color:C.textMid,fontStyle:"italic",marginTop:8,lineHeight:1.5,fontFamily:F.body}}>"{rawDic}"</div>}
            {cleaning&&<div style={{fontSize:12,color:C.accent,marginTop:8,fontFamily:F.body}}>Cleaning up dictation...</div>}
          </div>

          <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:14,marginBottom:14}}>
            <div style={{fontSize:9,color:C.accent,letterSpacing:2,marginBottom:8,fontFamily:F.body,textTransform:"uppercase"}}>Photo</div>
            {entry.photoPreview
              ? <div style={{position:"relative",marginBottom:8}}>
                  <img src={entry.photoPreview} style={{width:"100%",borderRadius:10,maxHeight:200,objectFit:"cover"}}/>
                  <button onClick={()=>setEntry(p=>({...p,photo:null,photoPreview:null}))} style={{position:"absolute",top:6,right:6,background:"#0a0a0acc",border:"none",color:"#fff",borderRadius:"50%",width:24,height:24,cursor:"pointer",fontSize:14}}>x</button>
                </div>
              : <div onClick={()=>photoRef.current?.click()} style={{border:`2px dashed ${C.sep}`,borderRadius:10,padding:"16px",textAlign:"center",cursor:"pointer",background:C.bg}}>
                  <div style={{fontSize:20,marginBottom:4}}>📷</div>
                  <div style={{fontSize:12,color:C.textMid,fontFamily:F.body}}>Add photo to entry</div>
                </div>
            }
          ...
          <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:14,marginBottom:14}}>
            <div style={{fontSize:9,color:C.accent,letterSpacing:2,marginBottom:8,fontFamily:F.body,textTransform:"uppercase"}}>Quick Signs</div>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={()=>{setShowForm(false);setQSigns([]);setRawDic("");}} style={{flex:1,background:"transparent",border:`1px solid ${C.sep}`,color:C.textMid,padding:"11px",borderRadius:24,fontSize:14,cursor:"pointer",fontFamily:F.body}}>Cancel</button>
          </div>
        </div>
      )}

      {journal.length===0&&!showForm&&(
        <div style={{textAlign:"center",padding:"40px 20px",color:C.textDim,fontSize:14,lineHeight:1.8,fontFamily:F.body}}>
      ...
      {journal.map(e=>(
        <JournalEntryCard
          key={e.id}
          entry={e}
          onDelete={()=>{const u=journal.filter(x=>x.id!==e.id);setJournal(u);saveLS("pl_journal",u);}}
          onUpdate={updated=>{const u=journal.map(x=>x.id===updated.id?updated:x);setJournal(u);saveLS("pl_journal",u);}}
        />
      ))}
    </div>
  );
...
export default function App() {
    ...
    try {
      if (awnKey && awnAppKey && awnMac) {
        ...
        try {
          ...
          // Pull Open-Meteo forecast even when using PWS (AWN has no forecast)
          try {
            const pos = await new Promise((res,rej)=>navigator.geolocation.getCurrentPosition(res,rej));
            const fcRes = await fetch("https://api.open-meteo.com/v1/forecast?latitude="+pos.coords.latitude+"&longitude="+pos.coords.longitude+"&daily=temperature_2m_min,temperature_2m_max,precipitation_sum&temperature_unit=fahrenheit&timezone=auto&forecast_days=10");
            const fcData = await fcRes.json();
            if (fcData.daily) {
              setForecast(fcData.daily.time.map((date,i)=>({
                date, minF:fcData.daily.temperature_2m_min[i], maxF:fcData.daily.temperature_2m_max[i], rain:fcData.daily.precipitation_sum[i]||0
              })));
            }
        ...
        ? navigator.geolocation.getCurrentPosition(
            async p => {
              try {
                ...
                if (w.daily) {
                  const fc = w.daily.time.map((date,i)=>({
                    date, 
                    minF: w.daily.temperature_2m_min[i],
                    maxF: w.daily.temperature_2m_max[i],
                    rain: w.daily.precipitation_sum[i]||0,
                  }));
                  setForecast(fc);
                }
                setWeather(wx);
  ...
  function useDemoData() {
    ...
    setForecast(Array.from({length:10},(_,i)=>{
      ...
      return {
        ...
        minF: 38 + Math.sin(i*0.8)*8 + Math.random()*4,
        maxF: 58 + Math.sin(i*0.6)*10 + Math.random()*5,
        rain: i===2||i===5 ? 0.4+Math.random()*0.6 : 0,
      };
    }));
  ...
  if (renderError) return (
    <div style={{padding:32,fontFamily:"sans-serif",color:"#333"}}>
      <h2>App Error</h2>
      <pre style={{fontSize:12,background:"#f5f5f5",padding:16,borderRadius:8,overflow:"auto"}}>{renderError}</pre>
      <button onClick={()=>setRenderError(null)} style={{marginTop:16,padding:"8px 16px",background:"#2D6A4F",color:"white",border:"none",borderRadius:8,cursor:"pointer"}}>Retry</button>
    </div>
  );

  return (
    <div style={{fontFamily:F.body,background:"#F2F2F7",height:"100dvh",color:C.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{background:"#2D6A4F",padding:"max(env(safe-area-inset-top,18px),18px) 20px 16px",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            ...
            <div style={{fontSize:10,color:"rgba(255,255,255,0.4)",marginTop:1,fontFamily:F.body}}>v{APP_VERSION}</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
      ...
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"16px 16px 24px"}}>
        {tab==="now"     && <Now     weather={weather} locationName={locName} loading={loading} apiError={apiErr} history={history} forecast={forecast}/>}
        {tab==="garden"  && weather && <Garden  weather={weather} history={history}/>}
        {tab==="garden"  && !weather && <div style={{padding:32,textAlign:"center",color:"#6C6C70",fontFamily:F.body}}>Loading weather data...</div>}
        {tab==="yard"    && weather && <Yard    weather={weather} history={history}/>}
        {tab==="yard"    && !weather && <div style={{padding:32,textAlign:"center",color:"#6C6C70",fontFamily:F.body}}>Loading weather data...</div>}
      ...
      {/* Tab bar */}
      <div style={{display:"flex",borderTop:`1px solid ${C.sep}`,background:"rgba(255,255,255,0.97)",flexShrink:0,paddingBottom:"env(safe-area-inset-bottom,0px)",overflowX:"auto"}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{
            flex:1,minWidth:52,padding:"8px 2px 10px",border:"none",
