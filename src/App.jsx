    ...
    <div>
      ...
      {weather && (
        <div style={css.card}>
          ...
          {history.length>2 && <Spark data={history.slice(-24).map(s=>s.pressure*0.02953)} color={C.accent}/>}
        </div>
      )}
        ...
        <div style={css.card}>
          ...
          {shift && (
            <div style={{marginTop:12,padding:"10px 12px",borderRadius:10,background:`${shift.color}14`,border:`1px solid ${shift.color}44`,display:"flex",alignItems:"flex-start",gap:8}}>
              <span style={{fontSize:16,flexShrink:0}}>{shift.type==="veering"?"🔄":"↩️"}</span>
      ...
      {history.length>2 && (
        <div style={css.card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            {[
              {data:history.slice(-24).map(s=>s.temp),              color:"#fcd34d", label:"Temp",     unit:"F",    dec:0},
              {data:history.slice(-24).map(s=>s.humidity),          color:C.info,    label:"Humidity", unit:"%",    dec:0},
              {data:history.slice(-24).map(s=>s.pressure*0.02953),  color:C.accent,  label:"Pressure", unit:'"',    dec:2},
              {data:history.slice(-24).map(s=>s.windSpeed),         color:"#c4b5fd", label:"Wind",     unit:" mph", dec:0},
            ].map(s=>(
              <div key={s.label}>
                ...
                <div style={{fontSize:12,color:s.color,fontFamily:F.num,marginTop:2,textAlign:"right"}}>{s.data[s.data.length-1]?.toFixed(s.dec)}{s.unit}</div>
              </div>
            ))}
          </div>
        </div>
