...
const C = {
  ...
  accent:"#0A7E8C",    // deep teal
  accentDim:"#E6F4F5", // teal tint for backgrounds
  accentMid:"#0D9CAC", // slightly lighter teal for hover states
...
const css = {
  ...
  btn:{ background:"#0A7E8C", border:"none", color:"#ffffff", padding:"11px 22px", borderRadius:980, fontSize:15, fontWeight:"600", cursor:"pointer", fontFamily:F.body },
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          max_tokens:400,
          messages:[{role:"user",content:
(() => { const dc = getDateContext(); return `You are a practical outdoor advisor. Give a direct, specific recommendation.
...
Give 2-3 sentences: confirm the verdict plainly, explain the most important condition factor, and if the verdict is wait or no say specifically when conditions would be right. Be direct and practical. No em dashes.`; })()
          }]
        })
      });
      const d = await res.json();
        ...
        body:JSON.stringify({
          ...
          messages:[{role:"user",content:
            (() => { const dc = getDateContext(); return (
              "You are a seasoned naturalist and field meteorologist. Data is from " + src + ".\n\n" +
              "DATE: " + dc.date + "\nTIME: " + dc.time + "\nSEASON: " + dc.season + " - Northern New Jersey\n\n" +
              "CURRENT CONDITIONS:\n" +
              ...
              "You are a seasoned naturalist in Northern New Jersey. Write 3-4 vivid sentences describing exactly what an experienced outdoorsman would observe right now given this specific date, time of day, season, and conditions. Be precise about what is actually present in NJ at this time of year: specific birds, insects, plants, smells, light quality. Then one confident 12-hour outlook sentence. No em dashes."tate, useEffect, useRef } from "react";
...
function getDateContext() {
  ...
  const timeOfDay = h<5?"overnight":h<9?"morning":h<12?"late morning":h<14?"midday":h<17?"afternoon":h<20?"evening":"night";
        ...
        body:JSON.stringify({
          ...
          messages:[{role:"user",content:
            (() => { const dc = getDateContext(); return (
              ...
              (weather.dew_point != null ? "- Dew point: " + Math.round(weather.dew_point) + "F\n" : "") +
              "- Pressure history: " + (phist || "first reading") + "\n\n" +
              "Write 3-4 vivid sentences as an experienced outdoorsman describing what they observe right now in " + dc.season + " in New Jersey - specific plants, birds, insects, smells, and sky conditions appropriate for " + dc.date + ". Then one confident 12-hour outlook sentence. Do not use em dashes."
            ); })()
          }]
        })
      });
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:F.body,background:"#F2F2F7",minHeight:"100dvh",color:C.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column"}}>
      ...
      {/* Header - solid teal */}
      <div style={{background:"#0A7E8C",padding:"max(env(safe-area-inset-top,18px),18px) 20px 16px",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
