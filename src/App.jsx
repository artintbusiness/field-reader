  ...
  if (editing) {
    return (
      <div style={{...css.card,borderColor:`${C.accent}44`}}>
        <div style={{fontSize:11,color:C.accent,fontFamily:F.body,marginBottom:8}}>{e.date} - {e.time}</div>
        {e.photo&&<img src={e.photo} style={{width:"100%",borderRadius:10,marginBottom:8,maxHeight:140,objectFit:"cover"}}/>}
        <textarea value={draftNote} onChange={ev=>setDraftNote(ev.target.value)}
          rows={3} style={{...css.input,resize:"vertical"}} placeholder="Notes..."/>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>{onUpdate({...e,note:draftNote});
                // Also sync to main journal
                ...
                setEditing(false);}} style={{...css.btn,flex:1,textAlign:"center",padding:"9px",fontSize:13}}>Save</button>
          <button onClick={()=>setEditing(false)} style={{flex:1,background:"transparent",border:`1px solid ${C.sep}`,color:C.textMid,padding:"9px",borderRadius:24,fontSize:13,cursor:"pointer",fontFamily:F.body}}>Cancel</button>
          <button onClick={onDelete} style={{background:"transparent",border:`1px solid ${C.danger}22`,color:C.danger,padding:"9px 14px",borderRadius:24,fontSize:13,cursor:"pointer",fontFamily:F.body}}>Delete</button>
        </div>
      </div>
    );
  }
          ...
          <div style={{background:C.surface,borderRadius:20,padding:24,maxWidth:320,width:"100%",boxShadow:"0 8px 32px rgba(0,0,0,0.2)"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{
                ...
                const newPlant={id:Date.now(),name:capName2,emoji:"🌱",location:"garden",planted:""};
