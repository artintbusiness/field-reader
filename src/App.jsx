...
function GardenChat({ weather, history, plants, tempOffset=0, onClose }) {
  ...
  function buildSystemPrompt() {
    const conditions = weather ? [
      `Temperature: ${Math.round(adjTemp)}F (yard-adjusted)`,
      `Humidity: ${weather.relative_humidity_2m}%`,
      `Soil est: ${soil}F`,
      `Rain 24hr: ${rain24.toFixed(2)}"`,
      `Wind: ${Math.round(weather.wind_speed_10m)} mph ${windLabel(weather.wind_direction_10m)}`,
      `Frost risk: ${frost?.label}`,
      `Source: ${weather.source==="pws"?"Personal backyard weather station":"Grid model"}`,
    ...
    return `You are a knowledgeable garden and plant diagnostician embedded in the Plot app. You have access to the user's current backyard conditions and plant journal history. Be specific, practical, and terse. Reference plant names and journal observations directly. Ask clarifying questions if needed. No em dashes.
