...
// ─── DESIGN TOKENS ────────────────────────────────────────────────────────────
const C = {
  bg:"#ffffff", surface:"#f5f7f5", card:"#f0f4f0", cardBorder:"#d0ddd0",
  accent:"#1a7a35", accentDim:"#d0eeda", text:"#1a2a1a", textMid:"#3a5a3a",
  textDim:"#7a9a7a", danger:"#c0392b", warn:"#b7770d", info:"#1a6a9a", good:"#1a7a35",
};
const F = { display:"'Playfair Display','Georgia',serif", body:"'DM Mono','Courier New',monospace" };
const css = {
  card:{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:16, padding:"18px 20px", marginBottom:14 },
  label:{ fontSize:17, letterSpacing:3, color:C.accent, textTransform:"uppercase", marginBottom:10, display:"block", fontFamily:F.body },
  input:{ width:"100%", background:"#ffffff", border:`1px solid ${C.cardBorder}`, borderRadius:10, padding:"12px 14px", color:C.text, fontSize:16, marginBottom:10, fontFamily:F.body, outline:"none", boxSizing:"border-box" },
  btn:{ background:C.accentDim, border:`1px solid ${C.accent}55`, color:C.accent, padding:"10px 18px", borderRadius:24, fontSize:17, cursor:"pointer", fontFamily:F.body, letterSpacing:1 },
  tag:(color)=>({ fontSize:17, padding:"3px 10px", borderRadius:20, background:`${color}22`, color, letterSpacing:1, fontFamily:F.body }),
};
...
function CondPills({ conditions }) {
  return (
    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:10}}>
      {conditions.map(c=>(
        <div key={c.label} style={{background:c.ok?"#e8f5ee":"#fff5f5",border:`1px solid ${c.ok?C.accentDim:"#f0c0c0"}`,borderRadius:8,padding:"5px 10px",display:"flex",gap:6,alignItems:"center"}}>
...
function Settings({ onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#00000066",zIndex:100,display:"flex",alignItems:"flex-end"}}>
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",background:"#ffffff",borderRadius:"20px 20px 0 0",padding:"24px 20px 40px"}}>
...
function PlantJournal({ plant, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#00000066",zIndex:100,display:"flex",alignItems:"flex-end"}}>
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",background:"#ffffff",borderRadius:"20px 20px 0 0",maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
          ...
          {showForm && (
            <div style={{...css.card,borderColor:`${C.accent}44`,marginBottom:16}}>
              ...
              {photoPreview
                ...
                : <div onClick={()=>photoRef.current?.click()} style={{border:`2px dashed ${C.cardBorder}`,borderRadius:10,padding:"12px",textAlign:"center",cursor:"pointer",background:"#ffffff",marginBottom:10}}>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={()=>{setShowForm(false);setNote("");setPhoto(null);setPhotoPreview(null);}} style={{flex:1,background:"transparent",border:`1px solid ${C.cardBorder}`,color:C.textMid,padding:"10px",borderRadius:24,fontSize:17,cursor:"pointer",fontFamily:F.body}}>Cancel</button>
              </div>
            </div>
          )}
...
function Garden({ weather, history }) {
  ...
  async function getAdvice() {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          messages:[{role:"user",content:
            "You are an expert gardener. Give specific, actionable advice for today based on the actual conditions and journal history.\n\n" +
            ...
            "Give 3-4 sentences of specific advice referencing plant names and recent journal notes where relevant. Include one concrete action to take today."
          }]
        })
      });
      ...
      else setAdvice("Could not parse response. Check API connection.");
    } catch(err) {
      setAdvice("Request failed: " + err.message);
    }
    setAdvLoad(false);
  }
  ...
  return (
    <div>
      ...
      <div style={{...css.card,borderColor:`${water.color}44`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          {[{l:"Humidity",v:Math.round(adjHum)+"%",n:offset>0?"adj":null},{l:"Rain 24hr",v:rain24.toFixed(2)+'"'},{l:"Soil Est.",v:soil+"F"}].map(s=>(
            <div key={s.l} style={{background:"#ffffff",borderRadius:10,padding:"8px 10px",textAlign:"center"}}>
      ...
      <div style={css.card}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
          {Object.entries(SEED_TEMPS).map(([name,s])=>{
            ...
            return (
              <div key={name} style={{background:"#ffffff",border:`1px solid ${col}33`,borderRadius:9,padding:"8px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      {showAdd&&(
        <div style={{...css.card,borderColor:`${C.accent}44`,marginBottom:16}}>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
            {PLANT_PRESETS.map(p=>(
              <button key={p.name} onClick={()=>setNp(prev=>({...prev,name:p.name,emoji:p.emoji}))} style={{padding:"6px 10px",borderRadius:20,fontSize:17,border:`1px solid ${np.name===p.name?C.accent:C.cardBorder}`,cursor:"pointer",fontFamily:F.body,background:np.name===p.name?C.accent+"18":"transparent",color:np.name===p.name?C.accent:C.textMid}}>{p.emoji} {p.name}</button>
            ))}
          </div>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={()=>setShowAdd(false)} style={{flex:1,background:"transparent",border:`1px solid ${C.cardBorder}`,color:C.textMid,padding:"11px",borderRadius:24,fontSize:17,cursor:"pointer",fontFamily:F.body}}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {plants.map(p=>(
        <div key={p.id} style={{background:"#ffffff",border:`1px solid ${C.cardBorder}`,borderRadius:12,padding:"12px 14px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
        ...
        <div style={css.card}>
          ...
          {showCal&&(
            <div style={{marginTop:12}}>
              ...
              {offset>0&&<div style={{marginTop:10,fontSize:17,color:C.accent,background:"#e8f5ee",padding:"8px 12px",borderRadius:8,fontFamily:F.body}}>Raw: {weather.relative_humidity_2m}% - Adjusted: {Math.round(Math.max(0,weather.relative_humidity_2m-offset))}%</div>}
            </div>
          )}
        </div>
      )}
...
function Yard({ weather, history }) {
  ...
  const groups = getYardConditions(weather, history);
  const toggle = id => setOpen(o=>({...o,[id]:!o[id]}));

  return (
    <div>
      <div style={{fontSize:17,color:C.textDim,marginBottom:14,lineHeight:1.7,fontFamily:F.body}}>
        Tasks surface based on your actual backyard conditions right now. Tap any group to expand.
        {weather.source==="pws"&&<span style={{color:C.accent}}> Using your PWS.</span>}
      </div>

      {groups.map(group=>(
        <div key={group.id} style={{...css.card,borderColor:`${group.color}44`,background:open[group.id]?`${group.color}08`:C.card}}>
          <div onClick={()=>toggle(group.id)} style={{cursor:"pointer"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:22}}>{group.emoji}</span>
                <div>
                  <div style={{fontSize:16,color:C.text,fontFamily:F.body,fontWeight:"bold"}}>{group.label}</div>
                  <div style={{fontSize:16,color:group.color,marginTop:2,fontFamily:F.body}}>{group.subtitle}</div>
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:17,color:C.textDim}}>{open[group.id]?"^":"v"}</span>
              </div>
            </div>
          </div>
          ...
          {open[group.id]&&(
            <div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${C.cardBorder}`}}>
              {group.tasks.map((task,i)=>(
                <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"10px 0",borderBottom:i<group.tasks.length-1?`1px solid ${C.cardBorder}`:"none"}}>
                  <span style={{width:8,height:8,borderRadius:"50%",background:task.ready?group.color:C.textDim,display:"block",flexShrink:0,marginTop:4,boxShadow:task.ready?`0 0 6px ${group.color}`:"none"}}/>
                  <div>
                    <div style={{fontSize:15,color:task.ready?C.text:C.textMid,fontFamily:F.body,marginBottom:3}}>{task.name}</div>
                    <div style={{fontSize:17,color:C.textDim,lineHeight:1.6,fontFamily:F.body}}>{task.detail}</div>
                  </div>
                </div>
              ))}
            </div>
...
function PhotoAI({ weather, history }) {
  ...
  async function analyze() {
    ...
    const prompts = {
      disease:
        "You are a plant pathologist and expert gardener. Analyze this photo for disease, pest damage, or nutritional deficiencies.\n\n" +
        "Current weather (" + src + "):\n" +
        "- Temperature: " + tmp + "F, Humidity: " + hum + "%, Soil temp est: " + soil + "F\n" +
        "- 24hr rainfall: " + rain24.toFixed(2) + '"\n\n' +
        "Provide: 1) Most likely diagnosis 2) Confidence level 3) What weather conditions suggest about cause 4) Specific treatment 5) What to watch for next. Be specific and practical.",
      product:
        "You are an expert agronomist and home improvement specialist. Read this product label carefully.\n\n" +
        "Current conditions (" + src + "):\n" +
        "- Air temp: " + tmp + "F, Humidity: " + hum + "%, Soil temp: " + soil + "F\n" +
        "- 24hr rainfall: " + rain24.toFixed(2) + '"\n' +
        "- Date: " + new Date().toLocaleDateString("en-US",{month:"long",day:"numeric"}) + "\n\n" +
        "Extract: 1) Product name and type 2) Application temperature range from label 3) Are current conditions within spec? 4) Rain/watering requirements 5) Direct yes/no: is today a good day to apply this product?",
    };
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",
      ...
      <div style={{...css.card,background:"linear-gradient(135deg,#e8f5ee,#f0faf2)",borderColor:`${C.accent}33`}}>
        ...
        {preview&&(
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            ...
            <button onClick={()=>{setPreview(null);setImage(null);setResult("");}} style={{background:"transparent",border:`1px solid ${C.cardBorder}`,color:C.textMid,padding:"12px 16px",borderRadius:24,fontSize:17,cursor:"pointer",fontFamily:F.body}}>Clear</button>
          </div>
        )}

        {result&&(
          <div style={{background:"#e8f5ee",border:`1px solid ${C.accentDim}`,borderRadius:12,padding:"14px 16px"}}>
...
function Now({ weather, locationName, loading, apiError, history }) {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          max_tokens:400,
          messages:[{role:"user",content:
            "You are a seasoned naturalist and field meteorologist. Data is from " + src + " in Northern New Jersey.\n\n" +
            "Current conditions:\n" +
            "- Temperature: " + Math.round(weather.temperature_2m) + "F\n" +
            "- Humidity: " + weather.relative_humidity_2m + "%\n" +
            "- Pressure: " + inHg + '" (' + trend.label + ")\n" +
            "- Wind: " + Math.round(weather.wind_speed_10m) + " mph from " + windLabel(weather.wind_direction_10m) + "\n" +
            (weather.uv != null ? "- UV Index: " + weather.uv + "\n" : "") +
            (weather.dew_point != null ? "- Dew point: " + Math.round(weather.dew_point) + "F\n" : "") +
            "- Pressure history: " + (phist || "first reading") + "\n\n" +
            "Write 3-4 vivid sentences as an experienced outdoorsman describing what they would observe right now - animals, plants, air quality, sky, smell, feel. Then one confident sentence on the 12-hour outlook. Do not use em dashes."
          }]
        })
      });
      const d = await res.json();
      ...
      else setAiText("Could not generate reading. Check connection.");
    } catch(err) {
  ...
  return (
    <div>
      {apiError&&<div style={{fontSize:16,color:C.warn,background:"#fffbf0",padding:"8px 12px",borderRadius:10,marginBottom:12,fontFamily:F.body}}>Warning: {apiError}</div>}

      {weather?.source==="pws"&&(
        <div style={{...css.card,borderColor:`${C.accent}33`,padding:"10px 14px",marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:16,color:C.accent,fontFamily:F.body,letterSpacing:2}}>YOUR BACKYARD PWS</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginTop:10}}>
            {[
              ...
              <div key={s.l} style={{background:"#ffffff",borderRadius:8,padding:"8px 6px",textAlign:"center"}}>
      ...
      {trend.trend==="falling_fast"&&(
        <div style={{background:"#fff0f0",border:`1px solid ${C.danger}`,borderRadius:14,padding:"12px 16px",marginBottom:12}}>
        ...
        <>
          <div style={css.card}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:44,height:44,borderRadius:"50%",border:`1px solid ${C.cardBorder}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,background:"#ffffff",flexShrink:0,transform:`rotate(${weather.wind_direction_10m+180}deg)`}}>^</div>
              <div>
        ...
        <div style={{...css.card,borderColor:`${C.accent}44`,marginBottom:16}}>
          ...
          <div style={{borderTop:`1px solid ${C.cardBorder}`,paddingTop:14,marginBottom:14}}>
            ...
            {entry.photoPreview
              ...
              : <div onClick={()=>photoRef.current?.click()} style={{border:`2px dashed ${C.cardBorder}`,borderRadius:10,padding:"16px",textAlign:"center",cursor:"pointer",background:"#ffffff"}}>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={()=>{setShowForm(false);setQSigns([]);setRawDic("");}} style={{flex:1,background:"transparent",border:`1px solid ${C.cardBorder}`,color:C.textMid,padding:"11px",borderRadius:24,fontSize:17,cursor:"pointer",fontFamily:F.body}}>Cancel</button>
          </div>
        </div>
      )}
...
export default function App() {
    ...
    try {
      if (awnKey && awnAppKey && awnMac) {
        ...
        try {
          ...
          const data = await res.json();
          if (!data || !data[0]) throw new Error("No AWN data");
          const d = data[0].lastData;
          const wx = {
            temperature_2m: d.tempf,
            relative_humidity_2m: d.humidity,
            ...
            feels_like: d.feelsLike ?? null,
            dew_point: d.dewPoint ?? null,
            source: "pws",
          };
          setWeather(wx);
  ...
  return (
    <div style={{fontFamily:F.body,background:C.bg,minHeight:"100dvh",color:C.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:"linear-gradient(160deg,#e8f5ee 0%,#f0f8f2 60%,#f5faf6 100%)",borderBottom:`1px solid ${C.cardBorder}`,padding:"max(env(safe-area-inset-top,18px),18px) 20px 14px",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontSize:17,letterSpacing:5,color:C.accent,textTransform:"uppercase",fontFamily:F.body}}>Plot</div>
            <div style={{fontFamily:F.display,fontSize:17,fontWeight:400,color:C.textMid,marginTop:2}}>
              {awnConfigured ? "Your Backyard PWS" : "Weather - Garden - Yard"}
            </div>
          </div>
          <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:17,color:C.textDim,marginBottom:2,fontFamily:F.body}}>
                {weather?.source==="pws" ? "📡 " : "📍 "}{locName||"Locating..."}
              </div>
              {weather&&<div style={{fontSize:36,color:C.text,fontFamily:F.display,fontWeight:400,lineHeight:1}}>{Math.round(weather.temperature_2m)}F</div>}
              {showFrostBadge&&<div style={{fontSize:17,color:frost.color,letterSpacing:1,marginTop:2,fontFamily:F.body}}>Frost: {frost.label}</div>}
            </div>
            <button onClick={()=>setShowSettings(true)} style={{background:"none",border:`1px solid ${C.cardBorder}`,color:C.textMid,borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:17,marginTop:2}}>⚙</button>
          </div>
        </div>
      </div>

      <div style={{display:"flex",borderBottom:`1px solid ${C.cardBorder}`,background:"#f0f4f0",flexShrink:0}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"9px 2px 8px",border:"none",background:tab===t.id?"#dceee3":"transparent",color:tab===t.id?C.accent:C.textDim,fontSize:11,cursor:"pointer",letterSpacing:0.5,borderBottom:tab===t.id?`2px solid ${C.accent}`:"2px solid transparent",fontFamily:F.body,transition:"all 0.15s",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
