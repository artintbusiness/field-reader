...
function PlantJournalSheet({ plant, allPlants, weather, onClose, onNewPlantDetected, onOpenChat }) {
  ...
  function saveEntry(entry) {
    ...
    const u = [entry, ...loadLS(key,[])]; // always read fresh from localStorage
    try {
      localStorage.setItem(key, JSON.stringify(u));
      // Verify the write actually worked
      const verify = localStorage.getItem(key);
      if (!verify) {
        alert("ERROR: localStorage write failed silently for key: " + key);
        return;
      }
      ...
      alert("ERROR saving entry: " + e.message + " | key: " + key);
      return;
    }
    setEntries(u);
    // Mirror to global journal
    const global = loadLS("pl_journal",[]);
    saveLS("pl_journal", [{ ...entry, plantId: plant.id, plantName: plant.name }, ...global]);
    idbSet(key, JSON.stringify(u)); // IDB async backup, non-blocking
    setShowForm(false);
    setSavedMsg(true);
