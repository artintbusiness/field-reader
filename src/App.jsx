...
function Garden({ weather, history, tempOffset=0, forecast=[], onOpenChat }) {
  ...
  function saveGeneralEntry(entry) {
    ...
    // Resolve detected plant names to actual plant objects
    const detectedNames = entry._detectedPlants || [];
    const matchedPlants = detectedNames
    ...
    const taggedEntry = {
      ...entryNoPhoto,
      plantNames: matchedPlants.length > 1 ? matchedPlants.map(p=>p.name) : undefined,
      plantName:  matchedPlants.length === 1 ? matchedPlants[0].name : undefined,
    };
    ...
    // Route to each detected plant's journal
