...
function wateringAdv(wx, hist, offset=0) {
  ...
  if ((wx.rain24h||0) > 0.5 || rain > 0.5) return { action:"skip",  label:"Skip watering",  color:C.good,    emoji:"💧", desc:((wx.rain24h||rain).toFixed(2)) + '" of rain recently.' };
...
function Garden({ weather, history }) {
  ...
  // General journal entry save
  function saveGeneralEntry(entry) {
    const global = loadLS("pl_journal",[]);
    saveLS("pl_journal", [entry, ...global]);
  ...
  return (
    <div>
      ...
      <div style={{marginTop:4,marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <span style={{...css.label,marginBottom:0}}>General Log</span>
            <InfoButton text="For observations not tied to one specific plant. Voice dictation will auto-detect plant mentions and link them."/>
          </div>
          <button onClick={()=>setShowGenJournal(o=>!o)} style={css.btnSoft}>+ Log</button>
        </div>
        {showGenJournal && (
          <JournalEntryForm
            weather={weather}
    ...
    <div>
      ...
      <div style={{...css.card,marginTop:4}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          ...
          <button onClick={()=>setShowProduct(o=>!o)} style={css.btnSoft}>{showProduct?"Hide":"Open"}</button>
        </div>
        {showProduct && (
          <>
...
function TenDayTrends({ history }) {
  ...
  const pMax=Math.max(...pressData.map(d=>d.val))+2, pMin=Math.min(...pressData.map(d=>d.val))-2;
  ...
  return (
    <div>
      ...
      <div style={css.card}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
          {pressData.map(d=>(<div key={d.day} style={{textAlign:"center",flex:1}}><div style={{fontSize:9,color:C.textDim,fontFamily:F.body}}>{d.day.split(" ")[1]}</div><div style={{fontSize:9,color:"#2D6A4F",fontFamily:F.body}}>{(d.val*0.02953).toFixed(2)}"</div></div>))}
        </div>
      </div>
    </div>
  );
    ...
    <div>
      ...
      {trend.trend==="falling_fast" && (
        <div style={{background:"#FFF2F2",border:`1px solid ${C.danger}`,borderRadius:14,padding:"12px 16px",marginBottom:10}}>
          ...
          <div style={{fontSize:14,color:C.textMid,lineHeight:1.5,fontFamily:F.body}}>Pressure dropped {Math.abs(trend.delta).toFixed(1)} hPa in 6 hours. Significant weather change likely soon.</div>
        </div>
      )}
        ...
        <div style={css.card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              ...
              <InfoButton text="Rapidly falling pressure (3+ hPa in 6hrs) means significant weather change imminent. Rising means improving."/>
            </div>
          </div>
          <div style={{fontSize:34,fontFamily:F.num,color:C.text,marginBottom:4,fontWeight:"200"}}>{inHg}<span style={{fontSize:14,color:C.textMid}}> inHg</span></div>
      ...
      {weather && (
        <div style={css.card}>
          <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:8}}>
            ...
            <InfoButton text="NW winds bring cold dry air. SW often precedes warm fronts."/>
          </div>
      ...
      {history.length>2 && (
        <div style={css.card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              <span style={css.label}>Live Trends</span>
              <InfoButton text="Last 24 readings. Spotting multi-reading patterns is more meaningful than a single value."/>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            {[
              ...
              <div key={s.label}>
                <div style={{fontSize:10,color:C.textDim,letterSpacing:2,textTransform:"uppercase",marginBottom:4,fontFamily:F.body}}>{s.label}</div>
                <Spark data={s.data} color={s.color}/>
                <div style={{fontSize:12,color:s.color,fontFamily:F.num,marginTop:2,textAlign:"right"}}>{s.data[s.data.length-1]?.toFixed(0)}{s.unit}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={css.card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:4,marginBottom:6}}>
            {forecast.slice(0,10).map((day,i)=>{
              const fr=frostRisk(day.minF,70,5); const isToday=i===0;
              return (
      ...
      if (awnKey && awnAppKey && awnMac) {
        try {
          ...
          try {
            ...
            const fc = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${pos.coords.latitude}&longitude=${pos.coords.longitude}&daily=temperature_2m_min,temperature_2m_max,precipitation_sum&temperature_unit=fahrenheit&timezone=auto&forecast_days=10`);
