          ...
          {history.length>2 && (()=>{
            ...
            return (
              <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:10}}>
                <div style={{fontSize:11,color:C.textDim,fontFamily:F.body,letterSpacing:0.5,textTransform:"uppercase",marginBottom:8}}>Recent Readings</div>
                {recent.map((s,i)=>{
      ...
      {weather && (
        <div style={css.card}>
          <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:10}}>
            ...
            <InfoButton text="Veering = clockwise rotation (e.g. NE→SE→S). Backing = counter-clockwise (e.g. S→SE→NE). Any significant shift of 45°+ is flagged. Direction toward S/SW typically means warmer, moister air. Toward N/NW means cooler, drier."/>
          </div>
          ...
          {/* Direction history — plain list, most recent first */}
          {history.length>2 && (()=>{
            const cardToDeg = {"N":0,"NNE":22.5,"NE":45,"ENE":67.5,"E":90,"ESE":112.5,"SE":135,"SSE":157.5,"S":180,"SSW":202.5,"SW":225,"WSW":247.5,"W":270,"WNW":292.5,"NW":315,"NNW":337.5};
            ...
            return (
              <div style={{borderTop:`1px solid ${C.sep}`,paddingTop:10}}>
                <div style={{fontSize:11,color:C.textDim,fontFamily:F.body,letterSpacing:0.5,textTransform:"uppercase",marginBottom:8}}>Direction History</div>
                {recent.map((s,i)=>{
                  // Arrow showing shift from previous reading
                  const prev = recent[i+1];
                  let shiftArrow = null;
                  if (prev && s.windDir && prev.windDir) {
                    ...
                    if (d1 != null && d2 != null) {
                      ...
                      if (diff > 180) diff -= 360;
                      if (diff < -180) diff += 360;
                      if (Math.abs(diff) >= 22.5) shiftArrow = diff > 0 ? "↻" : "↺";
                    }
                  }
                  return (
                    <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:i<recent.length-1?`1px solid ${C.sep}`:"none"}}>
                      ...
                      <span style={{fontSize:14,color:shiftArrow?"↻"===shiftArrow?C.warn:C.info:C.textDim,width:20,textAlign:"right"}}>
                        {shiftArrow||""}
                      </span>
                    </div>
                  );
          ...
          {/* Backing/veering alert */}
          {shift && (
            <div style={{marginTop:10,padding:"10px 12px",borderRadius:10,background:`${shift.color}14`,border:`1px solid ${shift.color}44`,display:"flex",alignItems:"flex-start",gap:8}}>
              <span style={{fontSize:15,flexShrink:0}}>{shift.type==="veering"?"↻":"↺"}</span>
