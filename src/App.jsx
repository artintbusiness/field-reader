...
function InfoButton({ text }) {
  ...
  return (
    <div style={{position:"relative",display:"inline-block"}}>
      ...
      {show&&(
        <>
          <div onClick={()=>setShow(false)} style={{position:"fixed",inset:0,zIndex:98}}/>
          <div style={{
            position:"absolute",right:0,bottom:28,zIndex:99,
            background:C.surface,border:`1px solid ${C.sep}`,
            borderRadius:12,padding:"12px 14px",width:240,
            boxShadow:"0 4px 16px rgba(0,0,0,0.12)",
          }}>
            <p style={{fontSize:13,color:C.textMid,lineHeight:1.6,fontFamily:F.body,margin:0}}>{text}</p>
          </div>
        </>
      )}
    </div>
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:F.body,background:"#F2F2F7",minHeight:"100dvh",color:C.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{fontSize:13,color:C.textMid,fontFamily:F.body,marginTop:3}}>{windLabel(weather.wind_direction_10m)} at {Math.round(weather.wind_speed_10m)} mph</div>
    </div>
  );
}